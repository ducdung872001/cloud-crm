...
| FE | **Date format sai** `"yyyy-MM-EEEEEETHH:mm:ss"` → `format(date, ...)` sinh chuỗi vô nghĩa `"2026-04-Mo177606900000015:30:00"` → BE trả 400 "Input wrong" (silent fail). Bug sao chép ở **12 files**: CreateReceipt, Warranty, Ticket, Loyalty, Setting, NetDeposit/NetLoan/NetServiceCharge (×2 branches CustomerAndSupplier + CustomerPerson) | Replace all: `"yyyy-MM-EEEEEETHH:mm:ss"` → `"yyyy-MM-dd'T'HH:mm:ss"` (12 files) |
