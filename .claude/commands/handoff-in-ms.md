---
description: Đọc reply từ microservice backend. Backend tạo issue mới trên repo cloud-crm với label reply-from-<service> + to-fitpro. Skill này list, tóm tắt, và đóng issue khi user đã xử lý.
argument-hint: "[issue-number | service-name | slug] — vd: /handoff-in-ms 42  hoặc  /handoff-in-ms customer"
---

Bạn đang đứng tại FE `fitpro` (nhánh `reborn-fitpro` trên repo `ducdung872001/cloud-crm`) và cần xử lý phản hồi từ 1 microservice backend. Backend đã hoàn tất task được giao (qua `/handoff-out-ms`) và mở 1 issue reply trên repo `ducdung872001/cloud-crm` với 2 label: `reply-from-<service>` AND `to-fitpro`.

> ⚠️ **Đặc thù fitpro**: repo `cloud-crm` dùng chung cho nhiều FE branch. PHẢI lọc bằng compound label (cả 2 label) để không nuốt phải reply của FE khác (banking, retail, mentorhub, …).
...
- **Rỗng** → list tất cả issue OPEN có CẢ HAI label `to-fitpro` AND match `reply-from-*` trên repo cloud-crm:
  ```bash
  ...
  gh issue list \
    ...
    --limit 50
  ```
  (gh search dùng OR ngầm cho nhiều label cùng prefix — kết hợp với `--label to-fitpro` để AND. Nếu output rỗng và bạn nghi có reply sót, fallback: `gh issue list --repo ducdung872001/cloud-crm --state open --label to-fitpro --json number,title,labels,url --limit 50` rồi filter client-side bằng regex `reply-from-*`.)
...
- **Khớp 1 trong 12 service** (`billing`, `bpm`, `care`, `contract`, `customer`, `integration`, `inventory`, `logistics`, `market`, `notification`, `operation`, `sales`) → list issue OPEN với label `reply-from-<service>` AND `to-fitpro`:
  ```bash
  gh issue list --repo ducdung872001/cloud-crm --state open \
    --label "reply-from-<service>" --label "to-fitpro" \
...
Nếu không tìm thấy → "Không có reply mới từ microservice nào cho fitpro." và STOP.

## Step 2 — Đọc & verify

`gh issue view <n> --repo ducdung872001/cloud-crm --json number,title,body,labels,state,url`

- Verify labels có CẢ `to-fitpro` AND pattern `reply-from-<service>`.
  - Nếu thiếu `to-fitpro` → cảnh báo "Issue #<n> không có label to-fitpro — có thể đây là reply cho FE khác (banking/retail/mentorhub). Skip để không đụng nhầm."
...
- **From service**: lấy từ label (vd `reply-from-customer` → `customer`)
- **Issue**: link `url`
- **Title**: title issue
...
a. **Comment + close issue reply**:
   ...
   gh issue close <n> \
     --repo ducdung872001/cloud-crm \
     --comment "$(cat <<'EOF'
   FE fitpro đã wire/áp dụng theo reply này.
...
- KHÔNG đụng vào issue thiếu label `to-fitpro` (có thể của FE khác trên cùng repo)
