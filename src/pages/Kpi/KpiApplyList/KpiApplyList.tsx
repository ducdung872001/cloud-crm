...
export default function KpiApplyList(props: any) {
  ...
  const onDeleteAll = () => {
    ...
    const arrPromises = selectedIds.map((selectedId) => {
      const found = listKpiApply.find((item) => item.id === selectedId);
      if (found?.id) {
        return KpiApplyService.delete(found.id);
      } else {
    ...
    .then((results) => {
      ...
      if (checkbox > 0) {
        showToast(`Xóa thành công ${checkbox} phiếu giao KPI`, "success");
        getListKpi(params);
        setListIdChecked([]);
      } else {
        showToast("Không có phiếu giao KPI nào được xóa", "error");
      }
   })
    .finally(() => {
      setShowDialog(false);
