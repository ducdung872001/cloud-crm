...
export default function AddQuoteForm({ onShow, onHide, data }) {
  ...
  const onSubmit = async (e) => {
    ...
    const response = await SheetQuoteFormService.update(body);

    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} mẫu báo giá thành công`, "success");
      onHide(true);
    } else {
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-add-quote-form"
      >
        <form className="form-quote-form-group" onSubmit={(e) => onSubmit(e)}>
