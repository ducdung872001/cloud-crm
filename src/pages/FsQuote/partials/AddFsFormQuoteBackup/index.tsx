...
export default function AddFsFormQuoteBackup(props: IAddFsQuoteProps) {
  const { idFS, onShow, onHide, dataFsQuote, disable } = props;
  ...
  const [lstFieldFSQuote, setLstFieldFSQuote] = useState([]);
  ...
  const handleLstFSQuoteService = async (id: number) => {
    const params = {
      fsId: id,
    };
  ...
  useEffect(() => {
    if (onShow && dataFsQuote) {
      handleGetField(dataFsQuote);
      handleLstFSQuoteService(idFS);
    }
  }, [onShow, dataFsQuote, idFS]);
  ...
  useEffect(() => {
    if (lstFieldConfig) {
      const mergeConditionData = mergeArrays(
        ...
        lstFieldFSQuote && lstFieldFSQuote.length > 0 ? lstFieldFSQuote : [defaultFieldConfig]
      );

      setLstField({ lstTbody: mergeConditionData, lstThead: lstFieldConfig.lstThead });
    }
  }, [lstFieldConfig, defaultFieldConfig, lstFieldFSQuote]);
  ...
  const handleDeleteRow = async (data, idx) => {
    ...
    if (idData) {
      const response = await FSQuoteService.fsFormDelete(idData);
  ...
  const handSubmitForm = () => {
    ...
    const sendRequest = async (item) => {
      ...
      await FSQuoteService.fsFormUpdate(body);
    };
    ...
    Promise.all(arrayPromise)
      .then((results) => {
        if (results && results.length > 0) {
          showToast("Cấu hình fs thành công", "success");
          setTimeout(() => {
            handClearForm(true);
          }, 500);
        }
      ...
      {!disable && lstField && (
        <div className="form-group">
          <div className="label__form">
            <span className="label">Thông tin FS</span>
