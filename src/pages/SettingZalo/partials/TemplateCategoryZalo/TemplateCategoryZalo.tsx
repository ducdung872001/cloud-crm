...
export default function TemplateCategoryZalo(props: any) {
  ...
  const onDelete = async (id: number) => {
    ...
    if (response.code === 0) {
      showToast(`Xóa chủ đề Zalo thành công`, "success");
      getListTemplateCategory();
    } else {
      showToast(response.message ?? "Có lỗi xảy ra. Vui lòng thử lại sau", "error");
    }
  ...
  const onDeleteAll = () => {
    ...
    .then((results) => {
      ...
      if (checkbox > 0) {
        showToast(`Xóa thành công ${checkbox} chủ đề Zalo`, "success");
        getListTemplateCategory();
        setListIdChecked([]);
      } else {
        showToast("Không có chủ đề Zalo nào được xóa", "error");
      }
   })
    .finally(() => {
      setShowDialog(false);
  ...
  const showDialogConfirmDelete = (item?: ITemplateCategoryResponseModel) => {
    const contentDialog: IContentDialog = {
      ...
      message: (
        <Fragment>
          Bạn có chắc chắn muốn xóa {item ? `chủ đề Zalo` : `${listIdChecked.length} chủ đề Zalo đã chọn`}
      ...
      <div className="card-box d-flex flex-column">
        ...
        {!isLoading && listTemplateCategory && listTemplateCategory.length > 0 ? (
          ...
          <SystemNotification
            description={
              <span>
                Hiện tại chưa có chủ đề Zalo nào. <br />
                Hãy thêm mới chủ đề Zalo nhé!
              </span>
            }
            type="no-item"
            titleButton={`Thêm mới chủ đề Zalo`}
            action={() => {
              setDataTemplateCategory(null);
              setShowModalAdd(true);
            }}
