...
export default function CollectInfoModal(props: any) {
  ...
  const getInfoEform = async (eformId, approachId, pipelineId, coyId) => {
    const params = {
      eformId: eformId,
    ...
    const response = await CampaignOpportunityService.OpportunityEformDetail(params);
  ...
  useEffect(() => {
    if (data && onShow) {
      getInfoEform(data.eformId, data.approachId, data.pipelineId, data.coyId);
    }
  }, [data, onShow]);
  ...
  const onSubmit = async (e) => {
    ...
    const response = await CampaignOpportunityService.OpportunityEformUpdate(body);

    if (response.code === 0) {
      showToast(`Thu thập thông tin biểu mẫu thành công`, "success");
      onHide(true);
    } else {
      showToast(response.message ?? "Có lỗi xảy ra. Vui lòng thử lại sau", "error");
      setIsSubmit(false);
