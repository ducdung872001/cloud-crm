...
import ModalManageInfo from "./partials/ModalManageInfo/ModalManageInfo";
import CollectInfoModal from "../CollectInfoModal/CollectInfoModal";
import CollectVOC from "../CollectVOC/CollectVOC";

export default function KanbanCompany(props: any) {
  const {
    data,
    kanbanTab,
    dataOfApproach,
  ...
  useOnClickOutside(refOption, () => setShowOptionExport(false), ["header_right"]);
  ...
  const onDragEnd = async (result) => {
    ...
    // const destColumnActivity = destColumn?.lstCampaignActivity?.map(item => {
    //     return item.code
    // }) || []
    ...
    if (startPoint !== endPoint) {
      if ((startPoint === null && endPoint === "done") || (startPoint === null && endPoint === "fail")) {
        ...
        //check đủ điều kiện kéo
        // checkChangeKanban(dragItem.id, sourceColumn.id)
  ...
  const handleUpdateStatusWork = async (endPoint, dragItem) => {
    const body: ICampaignOpportunityRequestModel = {
      ...
      type: dragItem.type,
      saleId: dragItem.saleId,
      opportunityId: dragItem.opportunityId,
    };
  ...
  const [showOptionExport, setShowOptionExport] = useState(false);
  const [showModalAddWork, setShowModalAddWork] = useState(false);

  const exportResult = async (campaignId, campaignName, fromTime, toTime) => {
    const params = {
      name: "",
    ...
    const campaignNameConvert = removeVietnameseTones(campaignName);
    ...
    if (response) {
      ...
      link.download = `${campaignNameConvert}_Ketqua.xlsx`;
      link.click();
    } else {
  ...
  const exportAction = async (campaignId, campaignName, fromTime, toTime) => {
    const params = {
      name: "",
      campaignId: campaignId,
      approachId: -1,
    ...
    if (response) {
      ...
      const link = document.createElement("a");
      link.href = URL.createObjectURL(blob);
      link.download = `${campaignNameConvert}_Tuongtac.xlsx`;
      link.click();
    } else {
      showToast("Có lỗi xảy ra. Vui lòng thử lại sau", "error");
    }
  ...
  const exportCustomer = async (campaignId, campaignName, fromTime, toTime) => {
    const params = {
      name: "",
      campaignId: campaignId,
      approachId: -1,
    ...
    if (response) {
      ...
      const link = document.createElement("a");
      link.href = URL.createObjectURL(blob);
      link.download = `${campaignNameConvert}_Khachhang.xlsx`;
      link.click();
    } else {
      showToast("Có lỗi xảy ra. Vui lòng thử lại sau", "error");
    }
  ...
  //quản lý thông tin báo cáo chiến dịch
  const [modalManageInfo, setModalManageInfo] = useState(false);
  ...
  return (
    <div className="wrapper-kanban-Pineline-Company">
      ...
      <div className="header_kanban">
        <div className="header_left">
          ...
          <Tippy content="Nhắn zalo">
            {customerIdlist.length > 0 ? (
              <div>
                <Icon name="Zalo" style={{ width: 30, marginRight: 20, cursor: "pointer" }} onClick={() => {}} />
              </div>
            ) : (
              <div>
                <Icon
        ...
        {permissions["CAMPAIGN_EXPORT"] == 1 && (
          <div className="header_right">
            <div style={{ display: "flex" }}>
              <AdvancedDateFilter updateParams={takeFromTimeAndToTime} />
              <Tippy content={"Báo cáo chiến dịch"}>
                <div
                  className="icon_export"
                  onClick={() => {
                    setModalManageInfo(true);
                    setInfoCampaign({
                ...
                >
                  <Icon name="CollectInfo" style={{ width: 20, height: 20 }} />
                </div>
              </Tippy>
              <div
                className="icon_export"
                onClick={() => {
                  setShowOptionExport(!showOptionExport);
                }}
              >
                <Icon name="Download" style={{ width: 20, height: 20 }} />
              </div>
            </div>

            {showOptionExport && (
              <div className="option-export" ref={refOption}>
                <div className="box-export">
                  <div
                    className="export-contact"
                    onClick={() => {
                      exportAction(data[0]?.campaignId, data[0]?.campaignName, fromTime, toTime);
                      setShowOptionExport(false);
                    }}
                  >
                    <span className="titile_export_contact">Xuất tương tác</span>
                  </div>
                  <div
                    className="export-result"
                    onClick={() => {
                      exportResult(data[0]?.campaignId, data[0]?.campaignName, fromTime, toTime);
                      setShowOptionExport(false);
                    }}
                  >
                    <span className="titile_export_result">Xuất kết quả</span>
                  </div>
                  <div
                    className="customer-type"
                    onClick={() => {
                      exportCustomer(data[0]?.campaignId, data[0]?.campaignName, fromTime, toTime);
                      setShowOptionExport(false);
                    }}
                  >
                    <span className="titile_customer-type">Xuất khách hàng</span>
                  </div>
                </div>
              </div>
            )}
                          ...
                          >
                            <div>
                              {column.convertRate ? (
                                ...
                                //     style={{ color: 'white' }}
                                // >{column.title} • <span style={{fontSize: 13, fontWeight:'400', cursor:'pointer'}}>{column.convertRate.toFixed(1)}%</span></span>
                                                                ...
                                                                >
                                                                    <div>
                                                                        <Icon
                                                                            ...
                                                                            style={{
                                                                                width: 18, marginRight: marginRight,
      ...
      <ModalManageInfo
        onShow={modalManageInfo}
        dataCampaign={infoCampaign}
        onHide={(reload) => {
          // if (reload) {
          //   onReload();
          // }
          setModalManageInfo(false);
        }}
      />
