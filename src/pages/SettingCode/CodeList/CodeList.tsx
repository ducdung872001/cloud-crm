...
export default function dsCodeList(props: any) {
  ...
  const onDeleteAll = () => {
    ...
    const arrPromises = selectedIds.map((selectedId) => {
      const found = listCode.find((item) => item.id === selectedId);
      if (found?.id) {
        return CodeService.delete(found.id);
      } else {
    ...
    .then((results) => {
      ...
      if (checkbox > 0) {
        showToast(`Xóa thành công ${checkbox} mã`, "success");
        getListCode(params);
        setListIdChecked([]);
      } else {
        showToast("Không có mã nào được xóa", "error");
      }
   })
    .finally(() => {
      setShowDialog(false);
