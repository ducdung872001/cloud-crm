...
export default function ConfigZalo(props: any) {
  ...
  const onSubmit = async (formData) => {
    ...
    if (response.code === 0) {
      showToast(`Cập nhật điều kiện Zalo thành công`, "success");
      onHide(true);
      setEditName(true);
      setNodePoint(null);
    } else {
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-condition-zalo"
        size="lg"
      >
        <form className="form-condition-zalo">
          <ModalHeader title={dataNode?.name} toggle={() => !isSubmit && handleClearForm()} />
          <ModalBody>
            <div className="container-condition-zalo">
