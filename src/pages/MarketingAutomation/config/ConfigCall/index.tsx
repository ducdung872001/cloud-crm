...
export default function ConfigCall(props: Record<string, unknown>) {
  ...
  const onSubmit = async (formData) => {
    ...
    if (response.code === 0) {
      showToast(`Cập nhật điều kiện Call thành công`, "success");
      onHide(true);
      setEditName(true);
      setNodePoint(null);
    } else {
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-condition-call"
        size="lg"
      >
        <form className="form-condition-call">
          <ModalHeader title={dataNode?.name} toggle={() => !isSubmit && handleClearForm()} />
          <ModalBody>
            <div className="container-condition-call">
