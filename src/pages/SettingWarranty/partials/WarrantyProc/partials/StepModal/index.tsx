...
import { IStepModalProps } from "model/warrantyStep/PropsModel";
import { IAction, IActionModal } from "model/OtherModel";
import { IWarrantyStepResponse } from "model/warrantyStep/WarrantyStepResponseModel";
...
import WarrantyStepService from "services/WarrantyStepService";
...
export default function StepModal(props: IStepModalProps) {
  ...
  const [listStep, setListStep] = useState<IWarrantyStepResponse[]>([]);
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [dataStep, setDataStep] = useState<IWarrantyStepResponse>(null);
  ...
  const getListStep = async () => {
    setIsLoading(true);
    const response = await WarrantyStepService.list({ procId: infoProc?.idProc });
  ...
  const dataMappingArray = (item: IWarrantyStepResponse, index: number) => [
    index + 1,
  ...
  const actionsTable = (item: IWarrantyStepResponse): IAction[] => {
    return [
      {
        title: "Sửa",
  ...
  const onDelete = async (id: number) => {
    const response = await WarrantyStepService.delete(id);
  ...
  const showDialogConfirmDelete = (item?: IWarrantyStepResponse) => {
