...
export default function FieldMannagement() {
  ...
  const onDeleteAll = () => {
    ...
    const arrPromises = selectedIds.map((selectedId) => {
      const found = listField.find((item) => item.id === selectedId);
      if (found?.id) {
        return FieldService.delete(found.id);
      } else {
    ...
    .then((results) => {
      ...
      if (checkbox > 0) {
        showToast(`Xóa thành công ${checkbox} lĩnh vực`, "success");
        getListField(params);
        setListIdChecked([]);
      } else {
        showToast("Không có lĩnh vực nào được xóa", "error");
      }
   })
    .finally(() => {
      setShowDialog(false);
