...
export default function UpgradePackageService(props: IUpgradePackageServiceProps) {
  const { data, dataInfoEmployee, onShow, onHide, callback } = props;
  ...
  const nameCommon = lstPackageService.find((item) => item.value === data?.packageType)?.label?.toLowerCase();
  ...
  const onsubmit = async (e) => {
    ...
    const body = {
      ...
      code: "CRM",
      packageId: dataExtend?.id,
    };

    const changeDataExtend = {
      price: dataExtend
        ? (dataExtend?.priceDiscount || 0) - (Math.abs(remainingMoney) || 0) <= 0
          ? 0
          : (dataExtend?.priceDiscount || 0) - (Math.abs(remainingMoney) || 0)
        : 0,
      nameOrg:
        dataExtend.packageType === 1
          ? "Gói miễn phí"
          : dataExtend.packageType === 2
          ? "Gói cơ bản"
          : dataExtend.packageType === 3
          ? "Gói bạc"
          : dataExtend.packageType === 4
          ? "Gói vàng"
          : "Gói kim cương",
      name: dataExtend.name,
      packageType: dataExtend.packageType,
      extend: `${dataExtend.period < 10 ? `0${dataExtend.period} tháng` : dataExtend.period > 100 ? "Vĩnh viễn" : `${dataExtend.period} tháng`}`,
    };
    ...
    if (response.code === 0) {
      ...
      callback(changeDataExtend, response.result);
    } else {
