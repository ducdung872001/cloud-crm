...
import { ShowInvoiceModalProps } from "model/invoice/PropsModel";
...
export default function ShowInvoiceModal(props: ShowInvoiceModalProps) {
  const { onShow, idInvoice } = props;
    ...
    () => ({
      actions_right: {
        buttons: [
          ...
          {
            title: "Chọn loại in", // In hóa đơn A4/A5
            type: "button",
            color: "primary",
            is_loading: isSubmit,
            callback: () => {
  ...
  return (
    <Fragment>
      <ModalReceipt isOpen={onShow} isFade={true} staticBackdrop={true} isCentered={true} toggle={() => !isSubmit} className="modal-see-receipt">
          ...
          <Fragment>
            <div ref={componentRef}>
              <ModalReceiptHeader
                ...
                toggle={() => !isSubmit && navigate("/invoice_order")}
              />
              <ModalBodyReceipt
                type="IV4"
                code={detaiInvoice?.invoiceCode}
                phone={detaiInvoice?.employeePhone}
                address={detaiInvoice?.inventoryAddress}
