...
import { IContactAttributeRequest, IContactAttributeFilterRequest } from "model/contactAttribute/ContactAttributeRequest";
import { AddContactAttributeModalProps } from "model/contactAttribute/PropsModel";
...
export default function AddContactAttributeModal(props: AddContactAttributeModalProps) {
  const { onShow, onHide, dataContactAttribute } = props;

  const refShowField = useRef();
  useOnClickOutside(refShowField, () => setShowFields(false), ["formula"]);

  const dataCheck = dataContactAttribute?.attributes && JSON.parse(dataContactAttribute?.attributes)
   ...
   */
  const onSelectOpenContactAttribute = async () => {
    setIsLoadingContactAttribute(true);

    const params: IContactAttributeFilterRequest = {
      isParent: 1
    }
    const response = await ContactAttributeService.list(params);
    ...
    setIsLoadingContactAttribute(false);
  };

  useEffect(() => {
    // if (data?.parentId) {
    //   onSelectOpenContactAttribute();
    // }
    ...
    if (data?.datatype == 'lookup' && data?.attributes) {
      setDetailLookup(JSON.parse(data?.attributes).refType || 'contact');
    }
      ...
      Object.keys(dataOption).forEach((key) => {
        (dataOption[key] || []).map(item => {
          if (item.datatype == 'number') {
            arrField.push("contactAttribute_" + item.fieldName);
          }
        });
      });
  ...
  const listFieldFirst = useMemo(
    ...
    [listContactAttribute, isLoadingContactAttribute, data]
  );
  ...
  const listFieldSecond = useMemo(
    () =>
      [
        ...
        //   key: onShow,
        //   required: false,
        //   options: listContactAttribute,
        //   onMenuOpen: onSelectOpenContactAttribute,
        //   isLoading: isLoadingContactAttribute,
        // },
        {
          name: "parentId",
          type: "custom",
    ...
    [
      listContactAttribute, 
      isLoadingContactAttribute, 
  ...
  const checkDuplicated = async (fieldName, valueFieldName) => {
    ...
    const response = await ContactAttributeService.checkDuplicated(param);
    if(response.code === 0){
  ...
  useEffect(() => {
    if (formData?.values['datatype'] == 'formula') {
      if (contactAttributeFields == null) {
  ...
  const onSubmit = async (e) => {
    ...
    const body: IContactAttributeRequest = {
      ...(formData.values as IContactAttributeRequest),
      ...(data ? { id: data.id } : {}),
      ...(
    ...
    const response = await ContactAttributeService.update(body);

    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} trường thông tin người liên hệ thành công`, "success");
      onHide(true);
      setAddFieldAttributes([{ value: '', label: '' }]);
      setDetailLookup('contact');
      setNumberFormat('');
      setCheckFieldName(false);
      setContactAttributeFields(null);
          ...
          },
          {
            ...
            disabled:
              ...
              (!isDifferenceObj(formData.values, values) &&
                ...
                (formData.values['datatype'] == 'lookup' ? (detailLookup === (dataCheck ? dataCheck?.refType : 'contact')) : true) &&
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-add-contact-attribute"
        size="lg"
      >
        <form className="form-contact-attribute-source" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader title={`${data ? "Chỉnh sửa" : "Thêm mới"} trường thông tin người liên hệ`}
            toggle={() => {
              if (!isSubmit) {
                clearForm()
              }
            ...
            <div className="list-form-group">
              ...
              {/* Trường hợp là formula */}
              {
                formData?.values['datatype'] == 'formula' ? <div className="form-group formula">
                  ...
                  {
                    showFields && <div className="formula-list">
                      {
                        contactAttributeFields && (contactAttributeFields || []).map(item => {
