...
import { ITipUserConfigRequest } from "model/tipUserConfig/TipUserConfigRequestModel";
import { IAddTipUserConfigModalProps } from "model/tipUserConfig/PropsModel";
...
import "./AddTipUserConfigModal.scss";

export default function AddGroupRosesModal(props: IAddTipUserConfigModalProps) {
  ...
  const onSubmit = async (e) => {
    ...
    setIsSubmit(true);
    const body: ITipUserConfigRequest = {
      ...(formData.values as ITipUserConfigRequest),
      ...(data ? { id: data.id } : {}),
    };
    ...
    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} hoa hồng thành công`, "success");
      onHide(true);
    } else {
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-add-tip-user-config"
      >
        <form className="form-tip-user-config" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader title={`${data ? "Chỉnh sửa" : "Thêm mới"} cấu hình hoa hồng`} toggle={() => !isSubmit && onHide(false)} />
          <ModalBody>
