...
export default function ModalDetailOrder(props: any) {
  ...
  const [listProduct, setListProduct] = useState([
    {
      title: 'Áo thun oversize unisex',
      des: 'SKU: AT-OVSZ-L-WH · Màu Trắng · Size L',
      value: '245.000₫',
      quantity: 1
    },
    {
      title: 'Áo thun oversize unisex',
      des: 'SKU: AT-OVSZ-M-BK · Màu Đen · Size M',
      value: '245.000₫',
      quantity: 1
    },
  ])
      ...
      <form className="form-detail-order" onSubmit={(e) => onSubmit(e)}>
        ...
        <div className="container-header">
          <div className="box-title">
            ...
            <div>
              <span style={{fontSize: 14, fontWeight: '400', color: 'var(--extra-color-30)'}}>{dataOrder?.app} · {dataOrder?.time}</span>
            </div>
          </div>
          <div className="right-header">
            <div className="status">
              <Badge
                text={dataOrder?.status === 1 ? "Chờ xử lý" : dataOrder?.status === 2 ? "Đang giao" : dataOrder?.status === 3 ? "Hoàn thành" : "Đã hủy"}
            ...
            {dataStep.map((item, index) => (
              <div key={index}>
                <div className="item-kanban">
                  <div className="step-kanban" style={{backgroundColor: item.step === 1 ? 'green' : item.step === 2 ? '#FF6633' : 'var(--extra-color-30)' }}>
                    {item.step === 1 ?  
                  ...
                  {item.step === 5 ? null : 
                    <div style={{display: 'flex', flexDirection: 'column', gap: '0.6rem 0', marginTop: '1.5rem'}}>
                      <div style={{width: '8rem', height: 2, backgroundColor: item.checked ? 'green' : 'var(--extra-color-20)', borderRadius: '10px'}}/>
                      <div style={{width: '8rem', height: 2, backgroundColor: item.checked ? 'green' : 'var(--extra-color-20)', borderRadius: '10px', marginLeft: '2rem'}}/>
                    </div>
                  }
                </div>
