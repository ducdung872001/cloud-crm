...
export default function UpdatePeopleInvolved(props: any) {
  ...
  const getListRelatedPeople = async () => {
    ...
    if (response.code === 0) {
      ...
      setJobAssign(result.manager ? result.manager : result?.assigners && result.assigners[0] ? result?.assigners[0] : null);
          ...
          <div className="view-info">
            ...
            <div className="view-recipient">
              <div style={{ display: "flex" }}>
                ...
                {data?.taskType === "assigned_task" && (data?.status === 0 || data?.status === 1) && data?.managerId === dataEmployee?.id ? (
