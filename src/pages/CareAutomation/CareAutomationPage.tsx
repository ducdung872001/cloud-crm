...
const MOCK_RULES: AutomationRule[] = [
  ...
  { id: 3, name: "Cảnh báo KH VIP chưa được gọi", trigger: "vip_no_call",  action: ["assign_staff"],  isActive: true,  runCount: 45,   lastRun: "20/03/2026", description: "Tạo task cho NV khi KH Vàng/Kim Cương chưa được liên hệ trong 14 ngày" },
  { id: 4, name: "Nhắc điểm sắp hết hạn",          trigger: "point_expiry", action: ["push","sms"],   isActive: true,  runCount: 89,   lastRun: "22/03/2026", description: "Gửi Push + SMS nhắc khi còn 7 ngày điểm tích lũy sẽ hết hạn" },
...
export default function CareAutomationPage(props: any) {
  ...
  // HeaderTabMenu gọi onBackProps() không có arg.
  // CustomerCarePage truyền handleBack(isBack: boolean) — nên cần wrap.
  ...
  return (
    <div className="page-content">
      {/* HeaderTabMenu — breadcrumb về "Chăm sóc khách hàng" */}
      <HeaderTabMenu
      ...
      <div className="promo-stats-grid">
        {[
          { label: "Kịch bản đang chạy", value: String(rules.filter(r => r.isActive).length), color: "green"  },
          { label: "Tổng kịch bản",      value: String(rules.length),                         color: "purple" },
          { label: "Đã chạy hôm nay",   value: "47",                                          color: "blue"   },
          { label: "Tỷ lệ thành công",  value: "94%",                                         color: "orange" },
        ].map(s => (
      ...
      {showCreate && (
        <div className="ca-create-form">
          ...
          <div className="ca-section-label" style={{ marginTop: 16 }}>Hành động thực hiện</div>
          ...
          <div className="cm-field" style={{ marginTop: 14 }}>
            ...
            <textarea
              ...
              placeholder="Mô tả ngắn về điều kiện và hành động..."
            />
          </div>
          ...
          <div className="ca-form-actions">
            ...
            <button
              ...
              onClick={() => { alert("Đã lưu kịch bản (demo)"); setCreate(false); }}
            >
              Lưu kịch bản
            </button>
          </div>
      ...
      <div className="ca-rules-list">
        {rules.map(rule => {
          ...
          return (
            <div
              key={rule.id}
              className={`ca-rule-card ${!rule.isActive ? "ca-rule-card--inactive" : ""}`}
            >
              <div className="ca-rule-card__left">
                ...
                <div className="ca-rule-actions-list">
                  {rule.action.map(a => {
                    ...
                    return (
                      <span key={a} className="ca-action-tag" style={{ background: ac.color + "15", color: ac.color }}>
                        {ac.label}
                      </span>
                    );
                  })}
