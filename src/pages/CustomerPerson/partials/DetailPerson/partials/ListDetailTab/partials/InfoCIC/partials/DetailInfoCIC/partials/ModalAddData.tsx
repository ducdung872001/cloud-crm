...
export default function ModalAddData({ onShow, onHide, dataProps, customerId }) {
  ...
  const onSubmit = async (e) => {
    ...
    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} khoản vay tại TCTD thành công`, "success");
      onHide(true);
    } else {
    ...
    <Fragment>
      ...
      >
        <form className="form-add-data" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader title={`${dataProps?.id ? "Chỉnh sửa" : "Thêm mới"} khoản vay tại các TCTD`} toggle={() => !isSubmit && handleClearForm(false)} />
          <ModalBody>
