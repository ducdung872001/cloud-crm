...
import { ICustomerEmailListProps } from "model/customerEmail/PropsModel";
...
import { ICustomerEmailResponseModel } from "model/customerEmail/CustomerEmailResponseModel";
import { showToast } from "utils/common";
import HistorySendService from "services/HistorySendService";
import AddCustomerEmailModal from "./partials/AddCustomerEmailModal";
import AddCustomPlaceholderModal from "./partials/CustomPlaceholder/AddCustomPlaceholderModal";
import "./CustomerEmailList.scss";

export default function CustomerEmailList(props: ICustomerEmailListProps) {
  const { dataCustomer, onShow, callBack } = props;

  const isMounted = useRef(false);
  const [isNoItem, setIsNoItem] = useState<boolean>(false);
  const [listCustomerEmail, setListCustomerEmail] = useState<ICustomerEmailResponseModel[]>([]);
  ...
  const [params, setParams] = useState<ICustomerEmailFilterRequest>({
  ...
  const getListCustomerEmail = async (paramsSearch: ICustomerEmailFilterRequest) => {
    setIsLoading(true);

    const response = await HistorySendService.historySendEmail(paramsSearch, abortController.signal);

    if (response.code === 0) {
      const result = response.result;
      setListCustomerEmail(result.items);
      setPagination({
        ...pagination,
        page: +result.page,
  ...
  return (
    <div className={`page-content page-customer-email${isNoItem ? " bg-white" : ""}`}>
      <div className="card-box d-flex flex-column">
        {/* <div className="action-header">
          <div className="title__actions">
            <ul className="menu-list">
              <li className="active">Danh sách lịch sử gửi email</li>
            </ul>
            <Tippy content="Gửi email" delay={[100, 0]} animation="scale-extreme">
        ...
        {!isLoading && listCustomerEmail && listCustomerEmail.length > 0 ? (
          <div className="list-history-email">
            {listCustomerEmail.map((item, idx) => (
              <div key={idx} className="item-history-email">
          ...
          <Fragment>
            {isNoItem && (
              <SystemNotification
                description={<span>Hiện tại chưa có lịch sử gửi email nào.</span>}
                type="no-item"
                titleButton="Gửi email"
                action={() => {
                  setShowModalAdd(true);
                }}
              />
