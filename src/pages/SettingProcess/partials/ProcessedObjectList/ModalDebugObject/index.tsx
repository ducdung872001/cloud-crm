...
export default function ModalDebugObject({ onShow, onHide, dataObject }) {
  ...
  useEffect(() => {
    if (dataObject && onShow) {
      setParams((preState) => ({ ...preState, potId: dataObject?.id }));
      setParamsForm((preState) => ({ ...preState, processId: processId }));
    }
  ...
  const showDialogConfirmDelete = (item?: any) => {
    const contentDialog: IContentDialog = {
      ...
      message: (
        <Fragment>
          Bạn có chắc chắn muốn xóa {item ? "" : `${listIdChecked.length}`} {tab === 1 ? "Node" : "Link"} đã chọn{" "}
          {item ? <strong>{tab === 1 ? item.name || item.nodeId : item.linkId}</strong> : ""}? Thao tác này không thể khôi phục.
        </Fragment>
      ),
      cancelText: "Hủy",
      cancelAction: () => {
