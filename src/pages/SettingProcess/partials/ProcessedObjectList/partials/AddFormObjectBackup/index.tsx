...
import ObjectService from "services/ProcessedObjectService";
// import SheetFieldObjectFormService from "services/SheetFieldObjectFormService";
...
export default function AddFormObjectBackup(props: IAddObjectProps) {
  const { idObject, onShow, onHide, dataObject, disable } = props;
  ...
  const [lstFieldObject, setLstFieldObject] = useState([]);
  ...
  const handleLstObjectService = async (id: number) => {
    const params = {
      ObjectId: id,
    };

    const response = null; // await ObjectService.ObjectFormLst(params);

    if (response.code === 0) {
      const result = transformArray(response.result);
      setLstFieldObject(result);
    } else {
  ...
  const handleGetField = async (dataProps) => {
    ...
    const response = null; //await SheetFieldObjectFormService.lst(params);
  ...
  useEffect(() => {
    if (onShow && dataObject) {
      handleGetField(dataObject);
      handleLstObjectService(idObject);
    }
  }, [onShow, dataObject, idObject]);
  ...
  useEffect(() => {
    if (lstFieldConfig) {
      const mergeConditionData = mergeArrays(
        ...
        lstFieldObject && lstFieldObject.length > 0 ? lstFieldObject : [defaultFieldConfig]
      );

      setLstField({ lstTbody: mergeConditionData, lstThead: lstFieldConfig.lstThead });
    }
  }, [lstFieldConfig, defaultFieldConfig, lstFieldObject]);
  ...
  const handleDeleteRow = async (data, idx) => {
    ...
    if (idData) {
      const response = null; // await ObjectService.ObjectFormDelete(idData);
  ...
  const handSubmitForm = () => {
    ...
    const sendRequest = async (item) => {
      ...
      // await ObjectService.ObjectFormUpdate(body);
    };
