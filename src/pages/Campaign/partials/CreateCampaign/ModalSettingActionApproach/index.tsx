...
export default function ModalSettingActionApproach(props: any) {
    ...
    const titles = ["STT", "Hành động", "Bắt buộc"];
    ...
    const dataMappingArray = (item: any, index: number) => [
        ...
        item.code === 'email' ? 'Gửi Email' 
            : item.code === 'sms' ? 'Gửi SMS' 
                : item.code === 'call' ? 'Gọi điện' 
                    : item.code === 'zalo' ? 'Gửi Zalo' 
                        : item.code === 'face' ? 'Gặp trực tiếp'
                            : item.code === 'schedule' ? 'Đặt lịch'
                                : item.code === 'contact' ? 'Đầu mối làm việc'
                                    : item.code === 'collectInfo' ? 'Thu thập thông tin'
                                        : item.code === 'voc' ? 'Thu thập VOC' : '',
        <div>
            {item.required === 1 ? 
    ...
    const showDialogConfirmDelete = (item?: any) => {
        const contentDialog: IContentDialog = {
          ...
          defaultText: "Xóa",
          defaultAction: async () => {
            const response = await CampaignApproachService.deleteActivity(item.id);
            if (response.code === 0) {
