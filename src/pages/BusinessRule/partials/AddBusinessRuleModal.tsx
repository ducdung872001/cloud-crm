...
import "./AddBusinessRuleModal.scss";
...
export default function AddBusinessRuleModal(props: any) {
  ...
  const listFieldBasic = useMemo(
    () =>
      [
        {
          label: "Tên loại loại luật nghiệp vụ",
          name: "name",
          type: "text",
          fill: true,
          required: true,
  ...
  const onSubmit = async (e) => {
    ...
    const response = await BusinessRuleService.update(body);

    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} loại luật nghiệp vụ thành công`, "success");
      onHide(true);
    } else {
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-add-business-rule"
        size="md"
      >
        <form className="form-add-reason-group" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader
            title={`${data ? "Chỉnh sửa" : "Thêm mới"} loại luật nghiệp vụ`}
            toggle={() => {
