...
import "./AddVehicleRegistrationModal.scss";
...
export default function AddVehicleRegistrationModal(props: any) {
  ...
  const [listVehicle, setListVehicle] = useState<IOption[]>([]);
  const fetchDataVehicle = async () => {
    ...
    if (response.code === 0) {
      const data = response.result.items.map((item) => {
        return {
          value: item.id,
          label: item?.customerName ? item.customerName + " - " + item.licensePlate : item.licensePlate,
        };
      });
      setListVehicle(data);
    }
  ...
  const listFieldBasic = useMemo(
    () =>
      [
        ...
        {
          label: "Loại phí đỗ xe",
          name: "feeType",
          type: "select",
          options: [
            {
              value: "monthly",
              label: "Phí đỗ xe tháng",
            },
            {
              value: "daily",
              label: "Phí đỗ xe ngày",
            },
          ],
          fill: true,
          required: true,
  ...
  const onSubmit = async (e) => {
    ...
    const response = await VehicleRegistrationService.update(body);

    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} đăng kí đậu xe thành công`, "success");
      onHide(true);
    } else {
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-add-vehicle_registration"
      >
        <form className="form-vehicle_registration-group" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader title={`${data ? "Chỉnh sửa" : "Thêm mới"} đăng kí đậu xe`} toggle={() => !isSubmit && onHide(false)} />
          <ModalBody>
            <div className="list-form-group">
              <div className="form-basic-info-vehicle_registration">
