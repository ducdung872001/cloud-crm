...
import "./OrtherFeeList.scss";
import moment from "moment";
import ParkingFeeService from "services/ParkingFeeService";
import AddOrtherFeeModal from "./partials/AddOrtherFeeModal";
import OrtherFeeService from "services/OrtherFeeService";

export default function OrtherFeeList(props: any) {
  document.title = "Danh sách chi phí khác";
  ...
  const listTabs = [
    {
      title: "Danh sách chi phí khác",
      is_active: "tab_one",
      type: 1,
    },
    // {
    //   title: "Sơ đồ phân cấp chi phí khác",
    //   is_active: "tab_two",
    //   type: 2,
    // },
  ];
  ...
  const getListElectricityRate = async (paramsSearch: any) => {
    ...
    const response = await OrtherFeeService.list(paramsSearch, abortController.signal);
  ...
  const titles = ["STT", "Tên khách hàng", "Mô tả chi phí", "Chi phí phải trả", "Kì thanh toán ", "Ngày cần thanh toán"];

  const dataFormat = ["text-center", "text-left", "text-left", "text-right", "text-center", "text-center"];

  const dataMappingArray = (item: any, index: number) => [
    getPageOffset(params) + index + 1,
    item.customerName || "",
    item.feeDescription,
    formatCurrency(item.amount, ".", " đ", "right"),
    moment(item.billingPeriod).format("DD/MM/YYYY"),
  ...
  const onDelete = async (id: number) => {
    const response = await OrtherFeeService.delete(id);

    if (response.code === 0) {
      showToast("Xóa chi phí khác thành công", "success");
      getListElectricityRate(params);
    } else {
      showToast(response.message ?? "Có lỗi xảy ra. Vui lòng thử lại sau", "error");
    }
  ...
  const showDialogConfirmDelete = (item?: any) => {
    const contentDialog: IContentDialog = {
      ...
      message: (
        <Fragment>
          Bạn có chắc chắn muốn xóa {item ? "chi phí khác " : `${listIdChecked.length} chi phí khác đã chọn`}
          {item ? <strong>{item.name}</strong> : ""}? Thao tác này không thể khôi phục.
        </Fragment>
      ),
      cancelText: "Hủy",
    ...
    <div className={`page-content page-branch${isNoItem ? " bg-white" : ""}`}>
      <div className="action-navigation">
        <div className="action-backup">
          <h1 className="title-first">Danh sách chi phí khác</h1>
        </div>
          ...
          <Fragment>
            {isPermissions ? (
              ...
              <SystemNotification
                description={
                  <span>
                    Hiện tại chưa có chi phí khác nào. <br />
                    Hãy thêm mới chi phí khác đầu tiên nhé!
                  </span>
                }
                type="no-item"
                titleButton="Thêm mới chi phí khác"
                action={() => {
                  setDataElectricityRate(null);
                  setShowModalAdd(true);
                }}
