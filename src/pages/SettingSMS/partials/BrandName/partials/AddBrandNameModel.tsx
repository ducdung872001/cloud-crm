...
import { IAddBrandNameModelProps } from "model/brandName/PropsModel";
import { IBrandNameRequestModel } from "model/brandName/BrandNameRequestModel";
import { useActiveElement } from "utils/hookCustom";
import Validate, { handleChangeValidate } from "utils/validate";
import { showToast } from "utils/common";
import { isDifferenceObj } from 'reborn-util';
import "./AddBrandNameModel.scss";
...
export default function AddBrandNameModel(props: IAddBrandNameModelProps) {
  ...
  const onSelectOpenPartner = async () => {
    if (!listPartner || listPartner.length === 0) {
      ...
      const response = await PartnerSMSService.list();
  ...
  const onSubmit = async (e) => {
    ...
    const body: IBrandNameRequestModel = {
      ...(formData.values as IBrandNameRequestModel),
      ...(data ? { id: data.id } : {}),
    };

    const response = await BrandNameService.update(body);

    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} BrandName thành công`, "success");
      onHide(true);
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-add-brandname"
      >
        <form className="form-brandname-group" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader
            title={`${data ? "Chỉnh sửa" : "Thêm mới"} BrandName`}
            toggle={() => {
