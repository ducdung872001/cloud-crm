...
export default function InstallApplication(props: any) {
  ...
  const onDeleteAll = () => {
    ...
    const arrPromises = selectedIds.map((selectedId) => {
      const found = listApp.find((item) => item.id === selectedId);
      if (found?.id) {
        return InstallApplicationService.delete(found.id);
      } else {
    ...
    .then((results) => {
      ...
      if (checkbox > 0) {
        showToast(`Xóa thành công ${checkbox} ứng dụng`, "success");
        getListApp(params);
        setListIdChecked([]);
      } else {
        showToast("Không có ứng dụng nào được xóa", "error");
      }
   })
    .finally(() => {
      setShowDialog(false);
