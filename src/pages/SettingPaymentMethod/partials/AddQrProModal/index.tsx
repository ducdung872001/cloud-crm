...
export default function AddQrProModal(props: IAddCategoryServiceModelProps) {
  ...
  const listField: IFieldCustomize[] = [
    ...
    {
      label: "Thời gian chờ thanh toán (phút)",
      name: "paymentTimeout",
      type: "text",
      fill: true,
      required: false,
  ...
  const onSubmit = async (e) => {
    ...
    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} phương thức thanh toán thành công`, "success");
      onHide(true);
    } else {
    ...
    <Fragment>
      ...
      >
        <form className="form-payment-method" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader title={`${data ? "Chỉnh sửa" : "Thêm mới"} phương thức thanh toán`} toggle={() => !isSubmit && onHide(false)} />
          <ModalBody>
