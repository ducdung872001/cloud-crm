...
export default function CollectInfoModal(props: any) {
  ...
  const getInfoEform = async (eformId, approachId, pipelineId, contractId) => {
    const params = {
      eformId: eformId,
    ...
    const response = await ContractEformService.ContractEformDetail(params);
  ...
  useEffect(() => {
    if (data && onShow) {
      getInfoEform(data.eformId, data.approachId, data.pipelineId, data.contractId);
    }
  }, [data, onShow]);
  ...
  const onSubmit = async (e) => {
    ...
    const response = await ContractEformService.ContractEformUpdate(body);
