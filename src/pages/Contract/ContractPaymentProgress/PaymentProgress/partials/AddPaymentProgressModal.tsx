...
export default function AddPaymentProgressModal(props: any) {
  ...
  const values = useMemo(
    () =>
      ({
        ...
        paymentBatch: data?.paymentBatch ?? "",
        attachments: data?.attachments ?? "",
        status: data?.status?.toString() ?? "0",
        taskAssignDate: data?.taskAssignDate ?? "",
        sendProfileDate: data?.sendProfileDate ?? "",
  ...
  const listFieldBasic = useMemo(
    () =>
      [
        ...
        {
          label: "Ngày bàn giao/nhận hồ sơ dự án",
          name: "taskAssignDate",
          type: "date",
          fill: true,
          required: false,
          // isMaxDate: true,
          placeholder: "Chọn ngày bàn giao/nhận hồ sơ",
          icon: <Icon name="Calendar" />,
          iconPosition: "left",
        },
        ...
        {
          label: "Ngày gửi hồ sơ cho khách hàng",
          name: "sendProfileDate",
          type: "date",
          fill: true,
          required: false,
          // isMaxDate: true,
          placeholder: "Chọn ngày gửi hồ sơ cho khách hàng",
          icon: <Icon name="Calendar" />,
          iconPosition: "left",
        },
  ...
  const onSubmit = async (e) => {
    ...
    // if (!formData.values.paymentDate) {
    //   showToast("Vui lòng nhập ngày thanh toán", "error");
    //   return;
    // }

    // if (!formData.values.amount) {
    ...
    if (response.code === 0) {
      showToast(`Thêm kỳ thanh toán thành công`, "success");
      setIsSubmit(false);
      onHide(true);
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-add-payment-progress"
        size="lg"
      >
        <form className="form-payment-progress-group" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader
            title={`${data ? "Chỉnh sửa" : "Thêm mới"} kỳ thanh toán`}
            toggle={() => {
              !isSubmit && handleClearForm();
            }}
          />
          <ModalBody>
            <div className="list-form-group-payment-progress">
