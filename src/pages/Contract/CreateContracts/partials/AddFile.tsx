...
export default function AddFile(props: IAddFileProps) {
  const { takeFileAdd, infoFile, setInfoFile, dataAttachment, notAddFile } = props;
  ...
  const download = (link, name) => {
    ...
    // Loại bỏ đuôi nếu name đã có đuôi
    const nameWithoutExtension = name.replace(/\.[^/.]+$/, "");
    const nameDownload = `${nameWithoutExtension}.${type}`;
      ...
      {!infoFile?.fileUrl ? (
        <div className="__action-operation">
          ...
          <div className="btn-upload--file">
            ...
            <input
              type="file"
              accept=".doc,.docx,.xls,.xlsx,.pdf,.PDF"
              className="d-none"
              id="uploadFile"
        ...
        <div className="show__file--upload">
          <div className="img__upload">
            ...
            <div style={{ width: "85%" }}>
              ...
              <h5 style={{ fontSize: 14 }}>{files[0]?.name ? files[0]?.name : `${infoFile?.fileName || "MauHopDong." + infoFile?.extension}`}</h5>
              {templateGetApi ? (
                <Tippy content="Tải xuống">
                  <div className="download" onClick={() => download(infoFile?.fileUrl, infoFile?.fileName || "MauHopDong")}>
