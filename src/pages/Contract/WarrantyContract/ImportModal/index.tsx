...
export default function ImportModal(props: IImportModalProps) {
  ...
  const handConfirmUploadFile = (data, parmas) => {
    if (!data) return;
    setIsSubmit(true);
    uploadDocumentFormData(data, onSuccess, onError, onProgress, "contractWarranty", +parmas);
  };
  ...
  const onSubmit = async (e) => {
    ...
    const response = await ContractWarrantyService.autoProcess(body);
  ...
  const handDownloadFileError = async () => {
    ...
    const response = await ContractWarrantyService.downloadFile(infoFile.uploadId);
    ...
    if (response) {
      ...
      link.download = `file_error_contract_warranty.xlsx`;
      link.click();
    } else {
  ...
  const initalFieldChoose = [
    { name: "Tên bảo hành(*)", fieldName: "name" },
    { name: "Hợp đồng gốc(*)", fieldName: "contractName" },
    { name: "Đơn vị thực hiện bảo hành(*)", fieldName: "competencyName" },
    { name: "Email đơn vị bảo hành(*)", fieldName: "competencyEmail" },
    { name: "Số điện thoại đơn vị bảo hành(*)", fieldName: "competencyPhone" },
    { name: "Đơn vị thụ hưởng(*)", fieldName: "beneficiaryName" },
    { name: "Email đơn vị thụ hưởng(*)", fieldName: "beneficiaryEmail" },
  ...
  const handGetExAttributes = async () => {
    ...
    const response = await ContractWarrantyService.exAttributes(params);
  ...
  const handSubmitDataTest = async (count: number, typeContract) => {
    ...
    const response = await ContractWarrantyService.numberFieldWarranty(body, param);
    ...
    if (response) {
      ...
      const link = document.createElement("a");
      link.href = URL.createObjectURL(blob);
      link.download = `Mau_nhap_bao_hanh.xlsx`;
      link.click();
    } else {
      showToast("Có lỗi xảy ra. Vui lòng thử lại sau", "error");
    }
                ...
                <div className="wrapper__step--two">
                  <div className="form-group">
                    <Input
                      ...
                      fill={true}
                      required={true}
                      placeholder="Nhập tên nguồn dữ liệu bảo hành"
