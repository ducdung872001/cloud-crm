...
import "./WarrantyAttachment.scss";
...
import WarrantyAttachmentService from "services/WarrantyAttachmentService";

export default function WarrantyAttachment(props: any) {
  ...
  useEffect(() => {
    if (warrantyId) {
      setParams((preState) => ({ ...preState, contractWarrantyId: warrantyId }));
    }
  }, [warrantyId]);
  ...
  const getListAttachment = async (paramsSearch: any) => {
    ...
    const response = await WarrantyAttachmentService.warrantyAttachmentList(paramsSearch);
  ...
  const showDialogConfirmDelete = (item?: any) => {
    const contentDialog: IContentDialog = {
      ...
      defaultText: "Xóa",
      defaultAction: async () => {
        const response = await WarrantyAttachmentService.warrantyAttachmentDelete(item.id);
        if (response.code === 0) {
