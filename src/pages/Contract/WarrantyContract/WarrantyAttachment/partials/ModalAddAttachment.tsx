...
export default function ModalAddAttachment(props: any) {
  const { onShow, onHide, data, warrantyId } = props;
  ...
  useEffect(() => {
    if (data) {
      setInfoFile({
        fileUrl: data.link,
        extension: data.link.includes(".docx")
          ? "docx"
          : data.link.includes(".xlsx")
          ? "xlsx"
          : data.link.includes(".pdf")
          ? "pdf"
          : data.link.includes(".pptx")
          ? "pptx"
          : data.link.includes(".zip")
          ? "zip"
          : "rar",
      });
    }
  ...
  const onSubmit = async (e) => {
    ...
    const response = await WarrantyAttachmentService.warrantyAttachmentUpdate(body);
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-add-warranty-attachment"
      >
