...
export default function SendEmailModal(props: any) {
  const { onShow, onHide, data, customerIdlist, dataQuote } = props;
  ...
  useEffect(() => {
    if (dataQuote && onShow) {
      handleCheckBase64(dataQuote, "export");
    }
  }, [onShow, dataQuote]);

  const handleCheckBase64 = async (item: any, type: "export") => {
    if (!item) return;
    ...
    if (response.code === 0) {
      ...
      if (result.length === 0) {
        showToast("Tài liệu báo giá không hợp lệ. Vui lòng cung cấp dữ liệu cho cấu hình báo giá và thử lại !", "warning");
      }
      ...
      if (result.length > 0) {
        ...
        if (isValid) {
          showToast(`Tài liệu báo giá Không hợp lệ để gửi. Dữ liệu cấu hình báo giá không được để trống !`, "warning");
        } else {
          ...
          CustomExportReport(
            {
              ...
              data: result.map((el) => dataMappingArrayExport(el)),
              info: { name },
            },
            "",
            setFilebase64,
  ...
  const onSubmit = async (e) => {
    ...
    setIsSubmit(true);
    const fileName = `${convertFileName(dataQuote?.name || "")}.xlsx`;
    ...
    const response = await ContractService.sendQuote(body);
  ...
  const titleExport = ["STT", "Nội dung", "Đơn vị", "Số lượng", "Đơn giá", "VAT", "Giá trị VAT", "Giá trị kế hoạch", "Diễn giải"];
  const formatExcel = ["center", "top", "top", "right", "right", "right", "right", "right", "top"];

  const dataMappingArrayExport = (item) => [
    item.orderLabel,
    item.content,
    item.unit,
    item.quantity,
  ...
  const handleExportDataFS = (data, nameProps: string) => {
    const changeName = removePunctuationAndCamelCase(nameProps);
    CustomExportReport({
      fileName: changeName,
      title: nameProps,
      header: titleExport,
      formatExcel: formatExcel,
      data: data.map((item) => dataMappingArrayExport(item)),
      info: { name },
    });
  };

  const handleCheckValidateExport = async (item: any, type: "export") => {
    if (!item) return;
    ...
    if (response.code === 0) {
      ...
      if (result.length > 0) {
        ...
        if (isValid) {
          showToast(`Không hợp lệ để tải xuống. Dữ liệu cấu hình báo giá không được để trống !`, "warning");
        } else {
          handleExportDataFS(result, item.name);
        }
      }
    } else {
      showToast("Có lỗi xảy ra. Vui lòng thử lại sau !", "error");
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-send-email-quote-contract"
        size="lg"
      >
        <form className="form-send-email-quote-contract" onSubmit={(e) => onSubmit(e)}>
          ...
          <ModalBody>
            <div className="wrapper-send-email">
              ...
              <div className="quote-attachment">
                <div>
                  <span style={{ fontSize: 16, fontWeight: "500" }}>Báo giá đính kèm</span>
                </div>
                <div className="img-document">
                  <div className="info-document">
                    <div className="__avatar--doc">
                      <img src={ImageExcel} alt={"tailieu"} />
                    </div>
                    <div className="__detail">
                      <span className="name-document">{convertFileName(dataQuote?.name || "")}.xlsx</span>
                      <Tippy content="Tải xuống">
                        <div
                          className="download"
                          onClick={() => {
                            handleCheckValidateExport(dataQuote, "export");
                          }}
                        >
                          <Icon name="Download" />
                        </div>
