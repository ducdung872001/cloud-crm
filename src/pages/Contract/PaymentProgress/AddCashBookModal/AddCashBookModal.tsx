...
export default function AddCashBookModal(props: any) {
  ...
  const values = useMemo(
    () =>
      ({
        // transDate: dataContractPayment?.paymentDate ?? "",
        ...
        amount: dataContractPayment?.amount - dataContractPayment?.paid ? dataContractPayment?.amount - dataContractPayment?.paid : "",
        ...
        projectId: dataContract?.projectId ?? "",
      } as ICashbookRequest),
    [dataCashBook, onShow, branchId, dataBranch, dataContractPayment, dataContract]
  );
  ...
  const listField = useMemo(
    () =>
      [
        ...
        //   fill: true,
        //   required: true,
        //   // disabled: dataContractPayment?.amount ? true : false
        // },
        {
          name: "amount",
          type: "custom",
          snippet: (
            <NummericInput
              ...
              onChange={(e) => {
                ...
                setFormData({ ...formData, values: { ...formData?.values, amount: value.replace(/,/g, "") } });
              }}
              placeholder="Tổng tiền từ hóa đơn"
              error={+formData.values.amount > dataContractPayment?.amount - dataContractPayment?.paid || !formData.values.amount}
              message={
                +formData.values.amount > dataContractPayment?.amount - dataContractPayment?.paid
                  ? `Số tiền nhập không được lớn hơn số tiền cần thanh toán: ${formatCurrency(
                      dataContractPayment?.amount - dataContractPayment?.paid
                    )}`
  ...
  const onSubmit = async (e) => {
    ...
    if (response.code === 0) {
      showToast(
        ...
        dataCashBook ? `Cập nhật đợt thanh toán thực tế thành công` : `Thêm mới đợt thanh toán thực tế thành công`,
        "success"
      );
      onHide(true);
      setDataCustomer(null);
  ...
  const actions = useMemo<IActionModal>(
    ...
    [formData, values, isSubmit, checkFieldEmployee, checkFieldCustomer, isShowCustomer, dataContractPayment]
  );
  ...
  const showDialogConfirmCancel = () => {
    const contentDialog: IContentDialog = {
      ...
      isCentered: true,
      isLoading: false,
      title: <Fragment>{`Hủy bỏ thao tác sửa đợt thanh toán thực tế`}</Fragment>,
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-contract-payment"
      >
        <form className="form-contract-payment-group" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader
            title={dataCashBook ? `Chỉnh sửa đợt thanh toán thực tế` : `Thêm mới đợt thanh toán thực tế`}
