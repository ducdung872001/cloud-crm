...
export default function ExpireTimeWarning(props: any) {
  ...
  const getDetailWarningSpecific = async (contractId) => {
    ...
    if (response.code === 0) {
      const result = response.result;
      const newData = result?.find((el) => el.type === "end_date") || null;
      const channelEmail = newData?.channels?.find((el) => el.type === "email") || null;
      const channelSms = newData?.channels?.find((el) => el.type === "sms") || null;
  ...
  const onSubmit = async (e) => {
    ...
    const body = {
      ...
      // alertType: 'end_date',
      ...(!dataContract ? { alertType: "end_date" } : { type: "end_date" }),
    ...
    // const response = await ContractService.contractAlertUpdate(body);
