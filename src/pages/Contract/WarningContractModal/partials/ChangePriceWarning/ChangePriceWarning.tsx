...
import "./ChangePriceWarning.scss";
...
export default function ChangePriceWarning(props: any) {
  ...
  const getDetailWarning = async () => {
    ...
    if (response.code === 0) {
      ...
      const newData = result?.find(el => el.alertType === 'adjust_date') || null;
      ...
      const employeeList = newData?.employees?.map(item => {
        return {
            value: item.id,
            label: item.name
        }
  ...
  const getDetailWarningSpecific = async (contractId) => {
    ...
    if (response.code === 0) {
      const result = response.result;
      const newData = result?.find(el => el.type === 'adjust_date') || null;
      const channelEmail = newData?.channels?.find(el => el.type === 'email') || null;
      const channelSms = newData?.channels?.find(el => el.type === 'sms') || null;
...
//   }, [addFieldPhone]);
  ...
  const onSubmit = async (e) => {
    ...
    const body = {
        ...
        ...(!dataContract 
          ? { employeeIds: formData.values.employeeList } 
          : { employeeIds: JSON.stringify(formData.values.employeeList)}
        ),
        ...(!dataContract 
          ? { sendTo: sendToData, } 
          : { sendTo: JSON.stringify(sendToData)}
        ),
