...
import "./PaymentProgressWarning.scss";
...
export default function PaymentProgressWarning(props: any) {
  ...
  const getDetailWarning = async () => {
    const response = await ContractService.contractAlertList();

    if (response.code === 0) {
      const result = response.result;
      const newData = result?.find((el) => el.alertType === "payment_period") || null;
  ...
  const getDetailWarningSpecific = async (contractId) => {
    const params = {
      type: "contract",
      objectId: 0,
      contractId: contractId,
    };
    const response = await ContractService.contractAlertListSpecific(params);

    if (response.code === 0) {
      const result = response.result;
      const newData = result?.find((el) => el.type === "payment_period") || null;
      const channelEmail = newData?.channels?.find((el) => el.type === "email") || null;
      const channelSms = newData?.channels?.find((el) => el.type === "sms") || null;
      ...
      setEmployeeList(employeeList);
      setSendToData(newData?.sendTo ? JSON.parse(newData?.sendTo) : []);
  ...
  useEffect(() => {
    if (!dataContract) {
      ...
      getDetailWarningSpecific(dataContract?.id);
    }
  }, [dataContract]);
  ...
  const onSubmit = async (e) => {
    ...
    const body = {
      ...
      ...(!dataContract ? { alertType: "payment_period" } : { type: "payment_period" }),
      ...
      // }
      ...(!dataContract ? { employeeIds: formData.values.employeeList } : { employeeIds: JSON.stringify(formData.values.employeeList) }),
      ...(!dataContract ? { sendTo: sendToData } : { sendTo: JSON.stringify(sendToData) }),
      ...(dataContract
        ? {
            contractId: dataContract.id,
            objectId: 0,
    ...
    let response = null;
    if (dataContract) {
      response = await ContractService.contractAlertSpecific(body);
    } else {
      response = await ContractService.contractAlertUpdate(body);
    }

    if (response.code === 0) {
      showToast(`Cập nhật cảnh báo thành công`, "success");
      setFormData({ ...formData, values: { ...formData?.values, id: response.result.id } });
  ...
  return (
    <div className="paymentProgress__form">
