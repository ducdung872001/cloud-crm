...
export default function ExchangeFast(props: IExchangeFastProps) {
  ...
  const handLstExchangeContent = async (params) => {
    const response = await ContractService.contractExchange(params);
  ...
  useEffect(() => {
    if (dataCustomer) {
      setParams({ ...params, contractId: dataCustomer.contractId });
    }
  }, [dataCustomer]);
  ...
  const onRemoveChat = async (id) => {
    const response = await ContractService.deleteContractExchange(id);
  ...
  return (
    <div className={`exchange__kanban__contract--fast ${actionHide ? "exchange__kanban__contract--fast--more" : ""}`}>
      {actionHide ? (
        <div className="item__small--exchange">
          <div className="icon-close-fast" onClick={() => onHide()}>
            <Icon name="TimesCircleFill" />
          </div>
          <Tippy content={dataCustomer.name} placement="left">
        ...
        <Fragment>
          <div className="header__exchange">
            <h4 className="title">{dataCustomer.name}</h4>
      ...
      <UploadMedia
        ...
        id={idExchange}
        contractId={dataCustomer ? dataCustomer.contractId : null}
        employeeId={dataEmployee ? dataEmployee.id : null}
        onHide={(reload) => {
          if (reload) {
            handLstExchangeContent(params);
