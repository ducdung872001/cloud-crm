...
export default function ContentChat(props: IContentChatProps) {
  const { dataMessage, contractId, employeeId, onHide } = props;
  ...
  useEffect(() => {
    if (employeeId || contractId) {
      setMessageChat({ ...messageChat, employeeId: employeeId, contractId: contractId });
    }
  ...
  useEffect(() => {
    if (dataMessage) {
      setMessageChat({
        ...
        contractId: dataMessage.contractId,
  ...
  const handleSendChat = async (e) => {
    ...
    const response = await ContractService.addContractExchange(body);
    ...
    <Fragment>
      <div className="wrapper__message--chat--exchange">
        ...
        <UploadMedia
          ...
          id={messageChat.id}
          contractId={messageChat.contractId}
