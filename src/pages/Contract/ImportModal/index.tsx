...
export default function ImportModal(props: IImportModalProps) {
  ...
  const handConfirmUploadFile = (data, parmas) => {
    if (!data) return;
    setIsSubmit(true);
    uploadDocumentFormData(data, onSuccess, onError, onProgress, 'contract', +parmas);
  };
  ...
  const onSubmit = async (e) => {
    ...
    const response = await ContractService.autoProcess(body);
  ...
  const handDownloadFileError = async () => {
    ...
    const response = await ContractService.downloadFile(infoFile.uploadId);
    ...
    if (response) {
      ...
      link.download = `file_error_contract.xlsx`;
      link.click();
    } else {
  ...
  const handDownloadFileSample = (file) => {
    ...
    xhr.onload = function () {
      ...
      if (xhr.status === 200) {
        ...
        link.download = "Mau_nhap_hop_dong.xlsx";
  ...
  const handGetExAttributes = async (typeContract) => {
    const params = {
        isCustomer: +typeContract,
    };

    const response = await ContractService.exAttributes(params);
  ...
  useEffect(() => {
    if (onShow && typeContract && typeContract) {
      handGetExAttributes(typeContract);
    }
  }, [onShow, typeContract, typeContract]);
  ...
  const handSubmitDataTest = async (count: number, typeContract) => {
    ...
    const response = await ContractService.numberFieldCustomer(body, param);
    ...
    if (response) {
      ...
      const link = document.createElement("a");
      link.href = URL.createObjectURL(blob);
      link.download = typeContract === "1" ? `Mau_nhap_hop_dong_khach_hang.xlsx` : `Mau_nhap_hop_dong_doi_tac.xlsx`;
      link.click();
    } else {
      showToast("Có lỗi xảy ra. Vui lòng thử lại sau", "error");
    }
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal__import--contract"
        size={`${infoFile?.data ? "xl" : "md"}`}
      >
        <form className="form__import--contract" onSubmit={(e) => onSubmit(e)}>
                  ...
                  <div className="download__sample--data">
                    ...
                    <div className="info__sample--data">
                      ...
                      <div className={`btn__submit--data ${hasSubmitDataTest ? "dis__submit--data" : ""}`} ref={refOptionContainer}>
                        <div className="btn__confirm" onClick={() => !hasSubmitDataTest && handSubmitDataTest(countDataTest, typeContract)}>
                          {hasSubmitDataTest ? (
                ...
                <div className="wrapper__step--two">
                  <div className="form-group">
                    <Input
                      ...
                      fill={true}
                      required={true}
                      placeholder="Nhập tên nguồn dữ liệu hợp đồng"
