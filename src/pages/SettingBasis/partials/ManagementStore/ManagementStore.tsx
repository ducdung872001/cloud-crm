...
import "./ManagementStore.scss";
import AddStoreModal from "./partials/AddStoreModal";
import EditParentStore from "./EditParentStore/EditParentStore";
import { IStoreResponse } from "@/model/managementStore/StoreResponseModel";
import { IStoreProps } from "@/model/managementStore/PropsModel";
...
export default function ManagementStore(props: IStoreProps) {
  ...
  const [listStore, setListStore] = useState<IStoreResponse[]>([]);
  const [dataBranch, setDataBranch] = useState<IStoreResponse>(null);
  ...
  const [params, setParams] = useState<IStoreFilterRequest>({
    name: "",
    limit: 10,
  });
  ...
  const getListStore = async (paramsSearch: IStoreFilterRequest) => {
    setIsLoading(true);
  ...
  const dataMappingArray = (item: IStoreResponse, index: number) => [
  ...
  const showDialogConfirmActiveBranch = (item?: IStoreResponse) => {
  ...
  const actionsTable = (item: IStoreResponse): IAction[] => {
  ...
  const showDialogConfirmDelete = (item?: IStoreResponse) => {
    const contentDialog: IContentDialog = {
      color: "error",
