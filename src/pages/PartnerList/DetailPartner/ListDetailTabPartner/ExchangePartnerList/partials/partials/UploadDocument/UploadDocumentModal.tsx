...
import { IUploadDocumentModalProps } from "model/mailBox/PropsModel";
...
export default function UploadDocumentModal(props: IUploadDocumentModalProps) {
  const { onShow, infoDocument, mailboxId, onHideForm, progress, onEditUpload, content, idItem } = props;
  ...
  useEffect(() => {
    if (isEditDocument && infoDocument) {
      const newData = [...lstData].map((item, index) => {
    ...
    <Fragment>
      ...
      >
        <form className="form__upload--document" onSubmit={(e) => handSubmitForm(e)}>
          <ModalHeader title={`Gửi tài liệu`} toggle={() => !isSubmit && onHideForm(false)} />
          <ModalBody>
