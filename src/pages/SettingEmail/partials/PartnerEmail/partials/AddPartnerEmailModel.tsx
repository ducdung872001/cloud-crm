...
import { IAddPartnerEmailModelProps } from "model/partnerEmail/PropsModel";
import { IPartnerEmailRequestModel } from "model/partnerEmail/PartnerEmailRequestModel";
...
import "./AddPartnerEmailModel.scss";

export default function AddPartnerEmailModel(props: IAddPartnerEmailModelProps) {
  ...
  useEffect(() => {
    if (data?.partnerConfig !== undefined) {
      const dataItem = data?.partnerConfig ? JSON.parse(data?.partnerConfig) : null;
      const changeDataItem = dataItem && Object.entries(dataItem).map((item) => {
        return {
          key: item[0],
          value: item[1],
        };
      ...
      if(changeDataItem && changeDataItem.length > 0){
  ...
  const onSubmit = async (e) => {
    ...
    const body: IPartnerEmailRequestModel = {
      ...(formData.values as IPartnerEmailRequestModel),
      ...(data ? { id: data.id } : {}),
    };

    const response = await PartnerEmailService.update(body);
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-add-partneremail"
      >
        <form className="form-partneremail-group" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader 
            ...
            <div className="list-form-group">
              ...
              <div className="list-field-config">
                ...
                {addFieldConfig.map((item, idx) => {
                  return (
                    <div key={item.value} className="field-config-item">
