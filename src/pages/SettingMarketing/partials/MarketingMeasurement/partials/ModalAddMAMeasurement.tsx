...
import "./ModalAddMAMeasurement.scss";
import CampaignMarketingService from "services/CampaignMarketingService";

export default function ModalAddMarketingMeasurement(props: any) {
  const { onShow, onHide, data, listMarketingMeasurement } = props;
  ...
  const values = useMemo(
    () =>
      ({
        ...
        position: data?.position ?? listMarketingMeasurement?.length + 1 ?? 0,
      } as any),
    [data, onShow, listMarketingMeasurement]
  );
  ...
  const onSubmit = async (e) => {
    ...
    const response = await CampaignMarketingService.updateMAMeasurement(body);

    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} đo lường thành công`, "success");
      onHide(true);
    } else {
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-add-marketing-measurement"
        size="lg"
      >
        <form className="form-add-marketing-measurement" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader
            title={`${data ? "Chỉnh sửa" : "Thêm mới"} đo lường`}
            toggle={() => {
