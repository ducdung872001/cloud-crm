...
import "./AddServiceAttributeModal.scss";
...
export default function AddServiceAttributeModal(props: any) {
  ...
  const checkDuplicated = async (fieldName, valueFieldName) => {
    ...
    const response = await ServiceAttributeService.checkDuplicated(param);
    if(response.code === 0){
  ...
  const onSubmit = async (e) => {
    ...
    const response = await ServiceAttributeService.update(body);

    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} trường thông tin dịch vụ thành công`, "success");
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-add-service-attribute"
        size="lg"
      >
        <form className="form-service-attribute-source" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader title={`${data ? "Chỉnh sửa" : "Thêm mới"} trường thông tin dịch vụ`} toggle={() => !isSubmit && handleClearForm(false)} />
          <ModalBody>
