...
import "./AddProductAttributeModal.scss";
...
export default function AddProductAttributeModal(props: any) {
  ...
  //? đoạn này xử lý vấn đề thay đổi danh mục
  ...
  const checkDuplicated = async (fieldName, valueFieldName) => {
    ...
    const response = await ProductAttributeService.checkDuplicated(param);
    if(response.code === 0){
  ...
  const onSubmit = async (e) => {
    ...
    const response = await ProductAttributeService.update(body);

    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} trường thông tin sản phẩm thành công`, "success");
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-add-product-attribute"
        size="lg"
      >
        <form className="form-product-attribute-source" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader title={`${data ? "Chỉnh sửa" : "Thêm mới"} trường thông tin sản phẩm`} toggle={() => !isSubmit && handleClearForm(false)} />
          <ModalBody>
