...
import "./AddCategoryServiceModal.scss";

export default function AddCategoryServiceModal(props: IAddCategoryServiceModelProps) {
  ...
  const onSubmit = async (e) => {
    ...
    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} danh mục dịch vụ thành công`, "success");
      onHide(true);
    } else {
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-add-category-service"
      >
        <form className="form-category-service" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader title={`${data ? "Chỉnh sửa" : "Thêm mới"} danh mục dịch vụ`} toggle={() => !isSubmit && onHide(false)} />
          <ModalBody>
            ...
            <FileUpload type="avatar" label="Ảnh danh mục dịch vụ" formData={formData} setFormData={setFormData} />
          </ModalBody>
