...
import "./AddContractAttributeModal.scss";
import ContractCategoryService from "services/ContractCategoryService";

export default function AddContractAttributeModal(props: AddContractAttributeModalProps) {
  ...
  const getContractAttributes = async (categoryId) => {
    ...
    const response = await ContractAttributeService.listAll(param);
  ...
  const loadedOptionParent = async (search, loadedOptions, { page }) => {
    ...
    const response = await ContractAttributeService.list(params);
  ...
  const checkDuplicated = async (fieldName, valueFieldName) => {
    ...
    const response = await ContractAttributeService.checkDuplicated(param);
    if (response.code === 0) {
  ...
  useEffect(() => {
    if (formData?.values["datatype"] == "formula" && formData?.values["categoryId"]) {
      if (contractAttributeFields == null) {
        getContractAttributes(formData?.values["categoryId"]);
      }
    }
  }, [formData]);
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-add-contract-attribute"
        size="lg"
      >
        <form className="form-contract-attribute-source" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader
            title={`${data ? "Chỉnh sửa" : "Thêm mới"} trường thông tin hợp đồng`}
            toggle={() => {
              if (!isSubmit) {
                handleClearForm();
              }
