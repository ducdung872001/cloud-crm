...
import "./AddGuaranteeAttributeModal.scss";
import GuaranteeAttributeService from "services/GuaranteeAttributeService";

export default function AddGuaranteeAttributeModal(props: any) {
  const { onShow, onHide, dataGuaranteeAttribute } = props;
  const refShowField = useRef();
  useOnClickOutside(refShowField, () => setShowFields(false), ["formula"]);

  const dataCheck = dataGuaranteeAttribute?.attributes && JSON.parse(dataGuaranteeAttribute?.attributes);
  ...
  useEffect(() => {
    setData(dataGuaranteeAttribute);
  }, [dataGuaranteeAttribute]);
  ...
  useEffect(() => {
    // if (data?.categoryId) {
    //   setDataCategory({ value: data.categoryId, label: data.categoryName })
    // }
   ...
   */
  const getGuaranteeAttributes = async (categoryId) => {
    const param = {
      categoryId: categoryId,
    };
    const response = await GuaranteeAttributeService.listAll(param);
  ...
  const loadedOptionParent = async (search, loadedOptions, { page }) => {
    ...
    const response = await GuaranteeAttributeService.list(params);
  ...
  const listFieldSecond = useMemo(
    ...
    [formData?.values["name"], formData?.values["fieldName"], formData?.values["datatype"], checkFieldName, detailParent]
  );
  ...
  const checkDuplicated = async (fieldName, valueFieldName) => {
    ...
    const response = await GuaranteeAttributeService.checkDuplicated(param);
    if (response.code === 0) {
  ...
  useEffect(() => {
    if (formData?.values["datatype"] == "formula" && formData?.values["categoryId"]) {
      if (contractAttributeFields == null) {
        getGuaranteeAttributes(formData?.values["categoryId"]);
      }
    }
  }, [formData]);
  ...
  const onSubmit = async (e) => {
    ...
    const response = await GuaranteeAttributeService.update(body);

    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} trường thông tin bảo lãnh thành công`, "success");
      onHide(true);
      setAddFieldAttributes([{ value: "", label: "" }]);
      setDetailLookup("contract");
      setNumberFormat("");
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-add-guarantee-attribute"
        size="lg"
      >
        <form className="form-guarantee-attribute-source" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader
            title={`${data ? "Chỉnh sửa" : "Thêm mới"} trường thông tin bảo lãnh`}
            toggle={() => {
              if (!isSubmit) {
                handleClearForm();
              }
                ...
                <div className="form-group formula">
                  ...
                  {showFields && (
                    <div className="formula-list" ref={refShowField}>
                      {contractAttributeFields && contractAttributeFields.length > 0 ? (
