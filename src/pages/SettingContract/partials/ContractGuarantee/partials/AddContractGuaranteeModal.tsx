...
import "./AddContractGuaranteeModal.scss";
...
export default function AddContractGuaranteeModal(props: any) {
  ...
  const onSubmit = async (e) => {
    ...
    const response = await ContractGuaranteeService.guaranteeTypeUpdate(body);

    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} loại bảo lãnh thành công`, "success");
      onHide(true);
    } else {
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-add-contract-guarantee"
      >
        <form className="form-contract-guarantee-group" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader
            title={`${data ? "Chỉnh sửa" : "Thêm mới"} loại bảo lãnh`}
            toggle={() => {
