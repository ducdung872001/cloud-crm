...
import "./AddContractApproach.scss";
...
export default function AddContractApproachModal(props: any) {
  const { onShow, onHide, data, dataContractPipeline, listContractApproach } = props;
  ...
  const values = useMemo(
    () =>
      ({
        id: data?.id ?? 0,
        name: data?.name ?? "",
        step: data?.step || listContractApproach?.length + 1 || 1,
        pipelineId: dataContractPipeline.id,
      } as any),
    [data, onShow, dataContractPipeline, listContractApproach]
  );
  ...
  const onSubmit = async (e) => {
    ...
    const response = await ContractApproachService.update(body);

    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} quy trình hợp đồng thành công`, "success");
      onHide(true);
    } else {
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-add-contract-approach"
      >
        <form className="form-contract-approach-group" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader
            title={`${data ? "Chỉnh sửa" : "Thêm mới"} quy trình hợp đồng`}
            toggle={() => {
