...
export default function AddBank(props: any) {
  ...
  const onSubmit = async (e) => {
    ...
    const response = await ContractGuaranteeService.bankUpdate(body);

    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} ngân hàng thành công`, "success");
      onHide(true);
    } else {
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-add-bank"
      >
        <form className="form-add-bank-group" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader
            title={`${data ? "Chỉnh sửa" : "Thêm mới"} ngân hàng`}
            toggle={() => {
