...
import "./AddWarrantyAttributeModal.scss";
import WarrantyAttributeService from "services/WarrantyAttributeService";

export default function AddWarrantyAttributeModal(props: any) {
  const { onShow, onHide, dataWarrantyAttribute } = props;
  const refShowField = useRef();
  useOnClickOutside(refShowField, () => setShowFields(false), ["formula"]);

  const dataCheck = dataWarrantyAttribute?.attributes && JSON.parse(dataWarrantyAttribute?.attributes);
  ...
  useEffect(() => {
    setData(dataWarrantyAttribute);
  }, [dataWarrantyAttribute]);
   ...
   */
  const getWarrantyAttributes = async (categoryId) => {
    const param = {
      categoryId: categoryId,
    };
    const response = await WarrantyAttributeService.listAll(param);
  ...
  const loadedOptionParent = async (search, loadedOptions, { page }) => {
    ...
    const response = await WarrantyAttributeService.list(params);
  ...
  const checkDuplicated = async (fieldName, valueFieldName) => {
    ...
    const response = await WarrantyAttributeService.checkDuplicated(param);
    if (response.code === 0) {
  ...
  useEffect(() => {
    if (formData?.values["datatype"] == "formula" && formData?.values["categoryId"]) {
      if (contractAttributeFields == null) {
        getWarrantyAttributes(formData?.values["categoryId"]);
      }
    }
  }, [formData]);
  ...
  const onSubmit = async (e) => {
    ...
    const response = await WarrantyAttributeService.update(body);

    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} trường thông tin bảo hành thành công`, "success");
      onHide(true);
      setAddFieldAttributes([{ value: "", label: "" }]);
      setDetailLookup("contract");
      setNumberFormat("");
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-add-warranty-attribute"
        size="lg"
      >
        <form className="form-warranty-attribute-source" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader
            title={`${data ? "Chỉnh sửa" : "Thêm mới"} trường thông tin bảo hành`}
            toggle={() => {
              if (!isSubmit) {
                handleClearForm();
              }
