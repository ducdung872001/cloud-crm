...
import "./AddContractCategoryModal.scss";
import ContractCategoryService from "services/ContractCategoryService";

export default function AddContractCategoryModal(props: any) {
  ...
  const values = useMemo(
    () =>
      ({
        ...
        profitThreshold: data?.profitThreshold ?? null,
  ...
  const onSubmit = async (e) => {
    ...
    const response = await ContractCategoryService.update(body);

    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} loại hợp đồng thành công`, "success");
      onHide(true);
    } else {
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-add-contract-category"
      >
        <form className="form-contract-category-group" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader
            title={`${data ? "Chỉnh sửa" : "Thêm mới"} loại hợp đồng`}
            toggle={() => {
