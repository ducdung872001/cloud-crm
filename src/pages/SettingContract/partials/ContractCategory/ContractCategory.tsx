...
export default function ContractCategoryList(props: any) {
  ...
  const [listContractCategory, setListContractCategory] = useState([]);
  const [dataContractCategory, setDataContractCategory] = useState(null);
  ...
  const getListContractCategory = async (paramsSearch: IContractPipelineFilterRequest) => {
    setIsLoading(true);

    const response = await ContractCategoryService.list(paramsSearch, abortController.signal);

    if (response.code === 0) {
      const result = response.result;
      setListContractCategory(result.items);
  ...
  useEffect(() => {
    ...
    if (isMounted.current === true) {
        getListContractCategory(params);
      const paramsTemp = _.cloneDeep(params);
  ...
  const titles = ["STT", "Tên loại hợp đồng", "Thứ tự"];
  ...
  const onDelete = async (id: number) => {
    const response = await ContractCategoryService.delete(id);
  ...
  const onDeleteAll = () => {
    ...
    const arrPromises = selectedIds.map((selectedId) => {
      const found = listContractCategory.find((item) => item.id === selectedId);
      if (found?.id) {
        return ContractCategoryService.delete(found.id);
      } else {
    ...
    <div className={`page-content page-contract-pipeline${isNoItem ? " bg-white" : ""}`}>
      <div className="action-navigation">
        <div className="action-backup">
          ...
          />
          <h1 className="title-last">Danh sách loại hợp đồng</h1>
        </div>
        <TitleAction title="" titleActions={titleActions} />
      </div>
      <div className="card-box d-flex flex-column">
        <SearchBox
          name="Tên loại hợp đồng"
          params={params}
          isSaveSearch={true}
          listSaveSearch={listSaveSearch}
          updateParams={(paramsNew) => setParams(paramsNew)}
        />
        {!isLoading && listContractCategory && listContractCategory.length > 0 ? (
          <BoxTable
            name="Loại hợp đồng"
            titles={titles}
            items={listContractCategory}
            isPagination={true}
      ...
      <AddContractCategoryModal
        onShow={showModalAddCategory}
        data={dataContractCategory}
        onHide={(reload) => {
          if (reload) {
            getListContractCategory(params);
          }
