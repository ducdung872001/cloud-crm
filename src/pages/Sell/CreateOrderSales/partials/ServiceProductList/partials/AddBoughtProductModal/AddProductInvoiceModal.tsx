...
export default function AddBoughtProductModal(props: AddBoughtProductModalProps) {
  const { onShow, onHide, invoiceId, idCustomer, data, dataSuggestedProduct } = props;
  ...
  const handTakeProduct = async (inventoryId: number, idProduct: number) => {
    ...
    const response = await WarehouseService.productList(params);

    if (response.code === 0) {
      const result = [...response.result.items].find((item) => item.productId === idProduct);

      const changeResult = {
        value: `${result.productId}_${result.batchNo}`,
        label: result.productName,
        avatar: result.productAvatar,
        productId: result.productId,
        lstWarehouse: result.lstWarehouse,
        batchNo: result.batchNo,
      };

      setDataProduct(changeResult);

      setFormData({
        ...formData,
        values: { ...formData?.values, inventoryId: inventoryId, productId: result.productId, batchNo: result.batchNo, price: 0 },
      });

      const resultWarehouse = (result.lstWarehouse || []).map((item) => {
        return {
          value: item.unitId,
          label: item.unitName,
          price: item.price,
      ...
      setListUnit(resultWarehouse);
    }
  };

  useEffect(() => {
    if (dataSuggestedProduct && onShow) {
      onSelectOpenInventory();
      handTakeProduct(dataSuggestedProduct.inventories[0].id, dataSuggestedProduct.id);
    }
  }, [dataSuggestedProduct, onShow]);
