...
export default function InvoiceDetailModal({ isOpen, data, onClose, onRefresh }: Props) {
  ...
  return (
    <>
      <Modal isOpen={isOpen} toggle={onClose} isCentered isFade className="idm-modal">
        <ModalHeader
          title={`Hóa đơn GTGT số ${data.invoiceNo || "Đang xử lý"}`}
          toggle={onClose}
          custom
        >
          <div className="idm__header">
            <div className="idm__header-left">
              <h4>Hóa đơn GTGT số {data.invoiceNo || <em style={{ color: "#9ca3af", fontStyle: "normal" }}>Đang xử lý</em>}</h4>
        ...
        {/* Custom footer (ModalFooter không hỗ trợ icon buttons) */}
        <div className="idm__footer">
          <Button color="secondary" variant="outline" onClick={handleDownloadPdf} disabled={loadingPdf} hasIcon>
            <Icon name="Download" />
            {loadingPdf ? "Đang tải..." : "Tải PDF"}
          </Button>
          <Button color="secondary" variant="outline" onClick={handleResendEmail} disabled={loadingEmail || !buyerEmail} hasIcon>
            <Icon name="Send" />
            {loadingEmail ? "Đang gửi..." : "Gửi lại email"}
          </Button>
          <Button color="secondary" variant="outline" onClick={() => setShowAdjust(true)} disabled={data.status !== "ISSUED"} hasIcon>
