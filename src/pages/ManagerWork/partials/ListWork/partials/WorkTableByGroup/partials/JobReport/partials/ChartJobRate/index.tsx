...
export default function ChartJobRate(props: ReportRevenueProps) {
  const { classNames, paramsProps, dataProjectProcess } = props;
  ...
  useEffect(() => {
    ...
    const totalWork = dataProjectProcess?.completedWork + dataProjectProcess?.remainingWork;
    const totalWorkload = dataProjectProcess?.workloadCompletion + dataProjectProcess?.workloadRemaining;
    const totalTime = dataProjectProcess?.timeCompletion + dataProjectProcess?.timeRemaining;

    setChartData({
      ...chartData,
      series: [
        // y là phần trăm: giá trị y ở chưa hoàn thành phải = (100 - giá trị y ở đã hoàn thành)
        // value là giá trị thật sự, unit là đơn vị hiển thị
        {
          name: "Chưa hoàn thành",
          data: [
            {
              y: (dataProjectProcess?.remainingWork / totalWork) * 100,
              value: dataProjectProcess.remainingWork,
              unit: "công việc",
            },
            {
              y: (dataProjectProcess?.workloadRemaining / totalWorkload) * 100,
              value: dataProjectProcess.workloadRemaining / 60,
              unit: "giờ",
            },
            { y: (dataProjectProcess?.timeRemaining / totalTime) * 100, value: dataProjectProcess.timeRemaining, unit: "ngày" },
          ],
        ...
        },
        {
          ...
          data: [
            {
              y: (dataProjectProcess?.completedWork / totalWork) * 100,
              value: dataProjectProcess.completedWork,
              unit: "công việc",
              id: "min",
            },
            {
              y: (dataProjectProcess?.workloadCompletion / totalWorkload) * 100,
              value: dataProjectProcess.workloadCompletion / 60,
              unit: "giờ",
            },
            { y: (dataProjectProcess?.timeCompletion / totalTime) * 100, value: dataProjectProcess.timeCompletion, unit: "ngày" },
          ],
  ...
  const [chartData, setChartData] = useState({
    ...
    xAxis: {
      categories: ["Số công việc", "Tổng workload", "Tổng thời gian"],
    },
    credits: {
      enabled: false,
    },
    yAxis: {
      ...
      stackLabels: {
        enabled: true, // hiển thị giá trị tổng
        format: "100%",
      },
    },
    tooltip: {
      headerFormat: "<b>{point.x}</b><br/>",
      pointFormat: "{series.name}: {point.value} {point.unit}<br/>",
    },
    legend: {
      reversed: true,
    },
    ...
    series: [
      ...
      // value là giá trị thật sự, unit là đơn vị hiển thị
      {
        name: "Chưa hoàn thành",
        data: [
          { y: 0, value: 0, unit: "công việc" },
          { y: 0, value: 0, unit: "MM" },
          { y: 0, value: 0, unit: "ngày" },
        ],
        color: chartColor.remaining,
      },
      {
        name: "Đã hoàn thành",
        data: [
          { y: 0, value: 0, unit: "công việc", id: "min" },
          { y: 0, value: 0, unit: "MM" },
          { y: 0, value: 0, unit: "ngày" },
        ],
        color: chartColor.success,
  ...
  return (
    <div className={`card-box report-revenue${classNames ? ` ${classNames}` : ""}`}>
      <div className="title__common d-flex align-items-start">
        <h2 className="name-common">Tiến trình dự án</h2>
        <div
