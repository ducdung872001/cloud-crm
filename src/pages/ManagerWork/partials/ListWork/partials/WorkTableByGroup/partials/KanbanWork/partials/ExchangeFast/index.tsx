...
import { showToast, handDownloadFileOrigin } from "utils/common";
...
export default function ExchangeFast(props: IExchangeFastProps) {
  ...
  useEffect(() => {
    if (dataWork) {
      setParams({ ...params, worId: dataWork.id });
    }
  }, [dataWork]);
  ...
  return (
    <div className={`exchange__kanban--fast ${actionHide ? "exchange__kanban--fast--more" : ""}`}>
      {actionHide ? (
        <div className="item__small--exchange">
          <div className="icon-close-fast" onClick={() => onHide()}>
            <Icon name="TimesCircleFill" />
          </div>
          <Tippy content={dataWork.name} placement="left">
        ...
        <Fragment>
          <div className="header__exchange">
            <h4 className="title">{capitalizeFirstLetter(dataWork.name)}</h4>
          ...
          >
            ...
            <div className="chat__exchange" style={lstDataExchange && lstDataExchange.length === 0 ? {} : {}}>
              <ContentChat
                dataMessage={showModalMedia || showModalDocument ? null : dataExchange}
                worId={dataWork.id}
      ...
      <UploadMedia
        ...
        id={idExchange}
        worId={dataWork ? dataWork.id : null}
        employeeId={dataEmployee ? dataEmployee.id : null}
        onHide={(reload) => {
          if (reload) {
            handLstExchangeContent(params);
