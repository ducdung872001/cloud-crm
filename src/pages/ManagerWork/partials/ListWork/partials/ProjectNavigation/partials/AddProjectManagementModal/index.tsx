...
export default function AddProjectManagementModal(props: IAddWorkProjectModalProps) {
  ...
  const getDetailProjectManagement = async (id: number) => {
    ...
    if (response.code === 0) {
      ...
      const dataLstProjectTypes = (result?.lstProjectType || []).map((el) => {
        return {
          value: el.id,
          label: el.name,
        };
      ...
      setDataProjectTypes(dataLstProjectTypes);
  ...
  //! đoạn này xử lý vấn đề lấy danh mục dự án
  ...
  const loadedOptionProjectTypes = async (search, loadedOptions, { page }) => {
    ...
    const response = await CategoryProjectService.list(param);
  ...
  const handleChangeValueProjectTypes = (data) => {
    setDataProjectTypes(data);

    const result = (data || []).map((item) => +item.value);
    setFormData({ ...formData, values: { ...formData.values, projectTypes: JSON.stringify(result) } });
  };
  ...
  const handleChangeValueDepartment = (e) => {
    ...
    setFormData({ ...formData, values: { ...formData?.values, departmentId: +e.value, employeeId: "" } });
  };
              ...
              <div className="list-form-group">
                ...
                <div className="form-group">
                  <SelectCustom
                    ...
                    value={dataProjectTypes}
                    onChange={(e) => handleChangeValueProjectTypes(e)}
                    loadOptionsPaginate={loadedOptionProjectTypes}
                    // formatOptionLabel={formatOptionLabelParticipant}
                  />
                </div>
                ...
                <div className="form-group">
                  <div className="attachments">
                    ...
                    <div className="list-file-scroll">
                        ...
                        <Fragment>
                          {listImageWork.map((item, idx) => (
                            <div key={idx} className="wrap-image-item">
                              ...
                              <div>{item?.name ? item?.name : `Tài liệu.${item?.type}`}</div>
                            </div>
                          ))}
