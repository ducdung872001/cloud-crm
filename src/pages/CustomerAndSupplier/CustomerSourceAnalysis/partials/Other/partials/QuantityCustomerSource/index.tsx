...
export default function QuantityCustomerSource(props) {
  ...
  const handGetQuantityCustomerSource = async () => {
    const param = {
      branchId,
    };

    const response = await CustomerService.classifyCustSource(param);
    ...
    if (chart && valueOption !== "excel") {
      ...
      chart.update({
        exporting: {
          ...
          filename: `${valueOption.startsWith("image") ? "Ảnh" : "Báo_cáo"}_theo_nguồn_khách_hàng`, // Tên file
