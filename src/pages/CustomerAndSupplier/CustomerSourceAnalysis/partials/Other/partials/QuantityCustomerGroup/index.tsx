...
export default function QuantityCustomerGroup(props) {
  ...
  const handGetQuantityCustomerGroup = async () => {
    const param = {
      branchId,
    };

    const response = await CustomerService.classifyCustGroup(param);
    ...
    if (chart && valueOption !== "excel") {
      ...
      chart.update({
        exporting: {
          ...
          filename: `${valueOption.startsWith("image") ? "Ảnh" : "Báo_cáo"}_theo_nhóm_khách_hàng`, // Tên file
