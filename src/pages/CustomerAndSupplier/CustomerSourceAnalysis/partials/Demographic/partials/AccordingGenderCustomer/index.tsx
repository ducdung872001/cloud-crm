...
export default function AccordingGenderCustomer(props) {
  ...
  const handGetGenderCustomer = async () => {
    const parma = {
      branchId,
    };

    const response = await CustomerService.classifyGender(parma);
    ...
    if (response.code === 0) {
      ...
      const changeResult = result.map((item) => {
        ...
        return {
          name: item.name == "female" ? "Nữ" : item.name == "male" ? "Nam" : "Khác",
          sum: item.count,
          y: roundedPercentage,
        };
      });
    ...
    if (chart && valueOption !== "excel") {
      ...
      chart.update({
        exporting: {
          ...
          filename: `${valueOption.startsWith("image") ? "Ảnh" : "Báo_cáo"}_theo_giới_tính_khách_hàng`, // Tên file
