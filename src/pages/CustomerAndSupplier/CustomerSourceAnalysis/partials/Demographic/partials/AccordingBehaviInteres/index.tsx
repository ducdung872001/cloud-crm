...
export default function AccordingBehaviInteres(props) {
  ...
  const handGetBehaviInteres = async () => {
    const param = {
      branchId,
    };

    const response = await CustomerService.classifyCustType(param);
    ...
    if (response.code === 0) {
      ...
      const changeResult = result.map((item) => {
        ...
        return {
          name: item.name.includes("doanh nghiệp") ? "Doanh nghiệp" : "Cá nhân",
          sum: item.count,
          y: roundedPercentage,
        };
      });
      ...
      showToast("Báo cáo theo cá nhân/tổ chức lỗi. Vui lòng xem lại sau!", "error");
    }
  };

  useEffect(() => {
    handGetBehaviInteres();
  }, []);
    ...
    if (chart && valueOption !== "excel") {
      ...
      chart.update({
        exporting: {
          ...
          filename: `${valueOption.startsWith("image") ? "Ảnh" : "Báo_cáo"}_theo_cá_nhân/tổ_chức`, // Tên file
  ...
  return (
    <div className="page__according__behavi--interes">
      <div className="title__common d-flex align-items-start">
        <h2 className="name-common">Theo cá nhân/tổ chức</h2>
        <div
