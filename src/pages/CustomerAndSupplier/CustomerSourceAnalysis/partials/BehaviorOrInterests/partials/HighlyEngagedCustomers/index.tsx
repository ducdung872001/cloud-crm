...
export default function HighlyEngagedCustomers(props) {
  ...
  const [chartData, setChartData] = useState({
    ...
    tooltip: {
      ...
      pointFormat: '<span style="color:{point.color}">Tổng</span>: <b>{point.sum} lần</b>',
    },
  ...
  const handGetEngagedCustomers = async () => {
    const param = {
      branchId,
      limit: 5,
    };

    const response = await CustomerService.classifyTopInteract(param);
    ...
    if (chart && valueOption !== "excel") {
      ...
      chart.update({
        exporting: {
          ...
          filename: `${valueOption.startsWith("image") ? "Ảnh" : "Báo_cáo"}_top_khách_hàng_tương_tác_nhiều`, // Tên file
