...
export default function TopSpendersCustomer(props) {
  ...
  const [chartData, setChartData] = useState({
    ...
    tooltip: {
      ...
      pointFormat: '<span style="color:{point.color}">Số lần mua</span>: <b>{point.sum} lần</b>',
    },
  ...
  const handGetSpendersCustomer = async () => {
    const param = {
      branchId,
      limit: 5,
    };

    const response = await CustomerService.classifyTopBought(param);
    ...
    if (chart && valueOption !== "excel") {
      ...
      chart.update({
        exporting: {
          ...
          filename: `${valueOption.startsWith("image") ? "Ảnh" : "Báo_cáo"}_top_khách_hàng_mua_nhiều_nhất`, // Tên file
          type: valueOption.split("/")[1], // Lấy phần mở rộng từ MIME typesss
        },
      });
