...
export default function HighValueCustomers(props) {
  ...
  const handGetValueCustomers = async () => {
    const param = {
      branchId,
      limit: 5,
    };

    const response = await CustomerService.classifyTopValueInvoice(param);
    ...
    if (chart && valueOption !== "excel") {
      ...
      chart.update({
        exporting: {
          ...
          filename: `${valueOption.startsWith("image") ? "Ảnh" : "Báo_cáo"}_top_khách_hàng_có_giá_trị_đơn_hàng_cao`, // Tên file
