...
import ModalSelectTypeZalo from "./ModalSelectTypeZalo/ModalSelectTypeZalo";

export default function CustomerZaloList(props: ICustomerZaloListProps) {
  const { idCustomer, customerName, onShow, callBack } = props;
  ...
  const [isSelectTypeZalo, setIsSelectTypeZalo] = useState(false);
  const [modalZNS, setModalZNS] = useState(false);
  const [typeZalo, setTypeZalo] = useState(null);
  ...
  useEffect(() => {
    if(onShow){
      setIsSelectTypeZalo(true);
    }

    // if (onShow && listCustomerZalo ) {
    //   if(listCustomerZalo.length <= 0){
    //     showAlert();
    //   } else {
    //     setShowModalAdd(true);
    //   }
    // }
  }, [onShow, listZaloFollowed]);
  ...
  return (
    <div className={`page-content page-customer-zalo${isNoItem ? " bg-white" : ""}`}>
      <div className="card-box d-flex flex-column">
        {!isLoading && listCustomerZalo && listCustomerZalo.length > 0 ? (
          ...
          <Fragment>
            {isNoItem && (
              <SystemNotification
                description={<span>Hiện tại chưa có lịch sử gửi Zalo nào.</span>}
                type="no-item"
                titleButton="Gửi Zalo"
                action={() => {
                  setShowModalAdd(true);
                  // if( listCustomerZalo && listCustomerZalo.length > 0) {
                  //   setShowModalAdd(true);
                  // } else {
                  //   showAlert();
                  // }
      ...
      <AddCustomerZaloModal
        onShow={showModalAdd || (onShow && listCustomerZalo.length > 0) || modalZNS}
        type= {typeZalo}
