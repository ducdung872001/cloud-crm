...
export default function AddCustomerZaloModal(props: IAddCustomerZaloModelProps) {
  const { onShow, idCustomer, onHide, callback, type } = props;
  ...
  const getListCategoryTemplateZalo = async (paramsSearch: ITemplateZaloFilterRequest, type) => {
    setIsLoading(true);

    let response = null;
    if(type ===  'zalo_oa'){
      response = await TemplateZaloService.list(paramsSearch);
    } else {
      response = await ZnsTemplateService.list(paramsSearch);
    }
  ...
  useEffect(() => {
    ...
    if (isMounted.current === true && onShow) {
        getListCategoryTemplateZalo(params, type);
  ...
  return (
    <Fragment>
      <Modal
        ...
        isCentered={true}
        staticBackdrop={true}
        toggle={() => !isSubmit && clearModal(false)}
        className="modal-send-zalo"
      >
        <form className="form-send-zalo" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader title={type === 'zalo_oa' ? 'Chọn mẫu gửi Zalo' : 'Chọn mẫu gửi Zalo ZNS'} toggle={() => !isSubmit && clearModal(false)} />
          <ModalBody>
                      ...
                      return (
                        <div key={idx} className={`${idCategoryZalo === item.id ? "active-template-zalo" : "item-template-zalo"}`}>
                          ...
                          {type === 'zalo_oa' ? 
                            ...
                            : 
                            <div className="info-template-zalo">
                                <h3 className="title">{trimContent(item.templateName, 100, true, true)}</h3>
                                {/* <span style={{fontSize: 14, marginBottom: 10}}>{`Chủ đề: ${item.tcyName}`}</span> */}
                            </div>
