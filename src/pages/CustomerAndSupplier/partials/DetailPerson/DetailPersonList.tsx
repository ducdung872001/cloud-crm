...
export default function DetailPersonList() {
  ...
  const takeUrlCustomerLocalStorage = (() => {
    try {
      return JSON.parse(localStorage.getItem("backUpUrlCustomer") || "{}");
    } catch {
      return {};
    }
  })();
    ...
    const result = (dataExtraInfo || [])
      .map((item) => {
        for (const key in dataAttribute) {
          const found = (dataAttribute[key] || []).find((o) => o.id === item.attributeId);
          if (found) return { value: item.attributeValue, label: found.name };
        }
        return null;
      })
      .filter(Boolean);
