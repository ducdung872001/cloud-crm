        ...
        <div className="retail-detail__body">
          ...
          <aside className="retail-detail__sidebar">
            ...
            <div className="rds-kpi-grid">
              ...
              <div
                className={`rds-kpi rds-kpi--clickable ${(invoiceStats?.debt ?? d.debt ?? 0) > 0 ? "rds-kpi--debt" : ""}`}
                onClick={() => { if ((invoiceStats?.debt ?? d.debt ?? 0) > 0) setShowKpiDebt(true); }}
                title={(invoiceStats?.debt ?? d.debt ?? 0) > 0 ? "Click để thu hồi công nợ" : undefined}
              >
                <span className="rds-kpi__label">
                  Công nợ
                  {(invoiceStats?.debt ?? d.debt ?? 0) > 0 && (
                    <span className="rds-kpi__collect-hint"> · Thu hồi</span>
                  )}
                </span>
                <span className={`rds-kpi__value ${(invoiceStats?.debt ?? d.debt ?? 0) > 0 ? "rds-kpi__value--danger" : ""}`}>
                  {parser(convertToPrettyNumber(invoiceStats?.debt ?? d.debt ?? 0))}
                </span>
              </div>
              ...
              <div className="rds-kpi">
                ...
                <span className="rds-kpi__value">
                  {invoiceStats?.completedCount ?? d.invoiceCount ?? 0}
                </span>
              </div>
