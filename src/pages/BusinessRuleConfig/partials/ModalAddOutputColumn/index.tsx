...
export default function ModalAddOutputColumn(props: any) {
  ...
  const onSubmit = async (e) => {
    ...
    const response = await DecisionTableOutputService.update(body);

    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} cột kết quả thành công`, "success");
      onHide(true);
    } else {
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-add-output-column"
        size="md"
      >
        <form className="form-add-reason-group" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader
            title={`${data ? "Chỉnh sửa" : "Thêm mới"} cột kết quả`}
            toggle={() => {
