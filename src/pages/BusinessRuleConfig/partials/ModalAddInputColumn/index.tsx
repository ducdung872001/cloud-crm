...
export default function ModalAddInputColumn(props: any) {
  ...
  const onSubmit = async (e) => {
    ...
    const response = await DecisionTableInputService.update(body);

    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} cột điều kiện thành công`, "success");
      onHide(true);
    } else {
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-add-input-column"
        size="md"
      >
        <form className="form-add-reason-group" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader
            title={`${data ? "Chỉnh sửa" : "Thêm mới"} cột điều kiện`}
            toggle={() => {
