...
import { IOpportunityExchangeResponseModal } from "model/campaignOpportunity/CampaignOpportunityResponseModel";
import { IOpportunityExchangeFilterRequest } from "model/campaignOpportunity/CampaignOpportunityRequestModel";
...
export default function ContentExchange(props: IContentExchangeWorkProps) {
  const { coyId, dataEmployee } = props;
  console.log("v", coyId);
  ...
  const [params, setParams] = useState<IOpportunityExchangeFilterRequest>({
    page: 1,
    limit: 10,
    coyId: coyId,
  });
  ...
  const [lstDataExchange, setLstDataExchange] = useState<IOpportunityExchangeResponseModal[]>([]);
  console.log("lstDataExchange", lstDataExchange);

  const [idExchange, setIdExchange] = useState<number>(0);
  const [dataExchange, setDataExchange] = useState<IOpportunityExchangeResponseModal>(null);

  const handLstExchangeContent = async () => {
    setIsLoading(true);

    const response = await CampaignOpportunityService.opportunityExchange(params);
  ...
  const onRemoveChat = async (id: number) => {
    ...
    const response = await CampaignOpportunityService.deleteOpportunityExchange(id);
  ...
  const showDialogConfirmDelete = (item: IOpportunityExchangeResponseModal) => {
  ...
  const handEditContent = async (id) => {
    ...
    const response = await CampaignOpportunityService.updateOpportunityExchange(id);
