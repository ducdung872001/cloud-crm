...
import "./ExchangeOpportunity.scss";
import MessageChatExchange from "./partials/MessageChatExchange";
import CampaignOpportunityService from "services/CampaignOpportunityService";
import Fancybox from "components/fancybox/fancybox";
import Image from "components/image";

export default function ExchangeOpportunity(props: any) {
  const { coyId } = props;
  ...
  const handleImageError = (event) => {
    event.target.src = ThirdGender;
  };

  return (
    <div className="wrapper-exchange-opportunity">
      <div className="info__exchange">
        <div className="info__exchange--body">
          {listExchange && listExchange.length > 0 && (
            <div className="wrapper__content--chat" onScroll={handleScroll}>
              <div className="content-chat">
                {listExchange.map((item, idx) => (
                  <div key={idx} className={`${item.employeeUserId === id ? "content__item--right" : "content__item--left"}`}>
                    <img src={item.employeeAvatar ? item.employeeAvatar : ThirdGender} alt="Employee Avatar" onError={handleImageError} />
