...
import "./MessageChatExchange.scss";
import { IMessageChatOpportunityRequestModal } from "model/campaignOpportunity/CampaignOpportunityRequestModel";
import CampaignOpportunityService from "services/CampaignOpportunityService";

export default function MessageChatExchange(props: any) {
  const { coyId, dataExchange, onReload } = props;
  ...
  const [messageChatExchange, setMessageChatExchange] = useState<IMessageChatOpportunityRequestModal>({
    content: "",
    media: null,
    coyId: coyId,
  });
  ...
  useEffect(() => {
    setMessageChatExchange({ ...messageChatExchange, coyId: coyId, content: "" });
  }, [coyId]);
  ...
  return (
    <Fragment>
      <div className="message-chat-exchange-opp">
