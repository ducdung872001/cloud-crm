...
export default function CollectVOC(props: any) {
  const { onShow, onHide, data, checkEmail} = props;
  ...
  const onSubmit = async () => {
    ...
    if(formData.values?.send === 'email'){
      if(data && data.customerEmail){
        handleClearForm(true, 'email');
      } else {
  ...
  const handleClearForm = (acc: boolean, type?) => {
    onHide(acc, type);
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-collect-voc"
      >
        <div className="container-collect-voc">
          <ModalHeader
            title={`Thu thập VOC qua`} 
            toggle={() => {
                if(!isSubmit){
                    handleClearForm(false);
                }
          ...
          <ModalBody>
            <div>
                <div className="list-form-group-voc">
