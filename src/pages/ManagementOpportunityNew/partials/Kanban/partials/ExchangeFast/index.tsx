...
export default function ExchangeFast(props: IExchangeFastProps) {
  ...
  useEffect(() => {
    if (dataCustomer) {
      setParams({ ...params, coyId: dataCustomer.coyId });
    }
  }, [dataCustomer]);
  ...
  return (
    <div className={`exchange__kanban__campaign--fast ${actionHide ? "exchange__kanban__campaign--fast--more" : ""}`}>
      {actionHide ? (
        <div className="item__small--exchange">
          <div className="icon-close-fast" onClick={() => onHide()}>
            <Icon name="TimesCircleFill" />
          </div>
          <Tippy content={`Thông tin trao đổi - ${dataCustomer.name}`} placement="left">
        ...
        <Fragment>
          <div className="header__exchange">
            <h4 className="title">Thông tin trao đổi - {dataCustomer.name}</h4>
      ...
      <UploadMedia
        ...
        id={idExchange}
        coyId={dataCustomer ? dataCustomer.coyId : null}
        employeeId={dataEmployee ? dataEmployee.id : null}
        onHide={(reload) => {
          if (reload) {
            handLstExchangeContent(params);
