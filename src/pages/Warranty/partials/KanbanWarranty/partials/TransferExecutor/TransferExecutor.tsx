...
import { ITransferExecutorProps } from "model/warranty/PropsModel";
...
export default function TransferExecutor(props: ITransferExecutorProps) {
  const { onShow, onHide, data, idStatusWarranty, idWarranty } = props;
  ...
  const values = useMemo(
    () =>
      ({
        id: idStatusWarranty ?? 0,
        executorId: data?.executorId ?? null,
        statusId: data?.statusId ?? 0,
        warrantyId: idWarranty ?? 0,
      } as IWarrantyProcessRequestModel),
    [onShow, data, idStatusWarranty, idWarranty]
  );
  ...
  const onSubmit = async (e) => {
    ...
    const body: IWarrantyProcessRequestModel = {
      ...(formData.values as IWarrantyProcessRequestModel),
    };

    const response = await WarrantyService.warrantyProcessUpdate(body);
