...
export default function ExchangeFast(props: IExchangeFastProps) {
  const { dataContact, onHide } = props;
  ...
  const handLstExchangeContent = async (params) => {
    const response = await ContactService.contactExchange(params);
  ...
  useEffect(() => {
    if (dataContact) {
      setParams({ ...params, contactId: dataContact.id });
    }
  }, [dataContact]);
  ...
  const onRemoveChat = async (id) => {
    const response = await ContactService.deleteContactExchange(id);
  ...
  return (
    <div className={`exchange__contact--fast ${actionHide ? "exchange__contact--fast--more" : ""}`}>
      {actionHide ? (
        <div className="item__small--exchange">
          <div className="icon-close-fast" onClick={() => onHide()}>
            <Icon name="TimesCircleFill" />
          </div>
          <Tippy content={dataContact.name} placement="left">
        ...
        <Fragment>
          <div className="header__exchange">
            <h4 className="title">{dataContact.name}</h4>
      ...
      <UploadMedia
        ...
        id={idExchange}
        contactId={dataContact ? dataContact.id : null}
        employeeId={dataEmployee ? dataEmployee.id : null}
        onHide={(reload) => {
          if (reload) {
            handLstExchangeContent(params);
