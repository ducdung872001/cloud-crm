...
export default function ContentChat(props: IContentChatProps) {
  const { dataMessage, contactId, employeeId, onHide } = props;
  ...
  useEffect(() => {
    if (employeeId || contactId) {
      setMessageChat({ ...messageChat, employeeId: employeeId, contactId: contactId });
    }
  ...
  useEffect(() => {
    if (dataMessage) {
      setMessageChat({
        ...
        contactId: dataMessage.contactId,
  ...
  const handleSendChat = async (e) => {
    ...
    const response = await ContactService.addContactExchange(body);
    ...
    <Fragment>
      <div className="wrapper__message--chat--exchange">
        ...
        <UploadMedia
          ...
          id={messageChat.id}
          contactId={messageChat.contactId}
