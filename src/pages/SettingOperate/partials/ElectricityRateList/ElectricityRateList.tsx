...
import "./ElectricityRateList.scss";
import AddElectricityRateModal from "./partials/AddElectricityRateModal";
import ElectricityRateService from "services/ElectricityRateService";
import moment from "moment";

export default function ElectricityRateList(props: any) {
  document.title = "Danh sách thang bậc giá điện";
  ...
  const [listSaveSearch] = useState<ISaveSearch[]>([
    {
      key: "all",
      name: "Danh sách thang bậc giá điện",
      is_active: true,
    },
  ]);
  ...
  const listTabs = [
    {
      title: "Danh sách thang bậc giá điện",
      is_active: "tab_one",
      type: 1,
    },
    // {
    //   title: "Sơ đồ phân cấp thang bậc giá điện",
    //   is_active: "tab_two",
    //   type: 2,
    // },
  ];
  ...
  const getListElectricityRate = async (paramsSearch: any) => {
    ...
    const response = await ElectricityRateService.list(paramsSearch, abortController.signal);
  ...
  const onDelete = async (id: number) => {
    const response = await ElectricityRateService.delete(id);

    if (response.code === 0) {
      showToast("Xóa thang bậc giá điện thành công", "success");
      getListElectricityRate(params);
    } else {
      showToast(response.message ?? "Có lỗi xảy ra. Vui lòng thử lại sau", "error");
    }
  ...
  const showDialogConfirmDelete = (item?: any) => {
    const contentDialog: IContentDialog = {
      ...
      message: (
        <Fragment>
          Bạn có chắc chắn muốn xóa {item ? "thang bậc giá điện " : `${listIdChecked.length} thang bậc giá điện đã chọn`}
          {item ? <strong>{item.name}</strong> : ""}? Thao tác này không thể khôi phục.
        </Fragment>
      ),
      cancelText: "Hủy",
    ...
    <div className={`page-content page-branch${isNoItem ? " bg-white" : ""}`}>
      <div className="action-navigation">
        <div className="action-backup">
          ...
          />
          <h1 className="title-last">Danh sách thang bậc giá điện</h1>
        </div>
          ...
          <Fragment>
            {isPermissions ? (
              ...
              <SystemNotification
                description={
                  <span>
                    Hiện tại chưa có thang bậc giá điện nào. <br />
                    Hãy thêm mới thang bậc giá điện đầu tiên nhé!
                  </span>
                }
                type="no-item"
                titleButton="Thêm mới thang bậc giá điện"
                action={() => {
                  setDataElectricityRate(null);
                  setShowModalAdd(true);
                }}
