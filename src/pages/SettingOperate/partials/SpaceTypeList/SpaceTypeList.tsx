...
import "./SpaceTypeList.scss";
import AddSpaceTypeModal from "./partials/AddSpaceTypeModal";
import SpaceTypeService from "services/SpaceTypeService";

export default function SpaceTypeList(props: any) {
  document.title = "Danh sách loại căn hộ";
  ...
  const listTabs = [
    {
      title: "Danh sách loại căn hộ",
      is_active: "tab_one",
      type: 1,
    },
    // {
    //   title: "Sơ đồ phân cấp loại căn hộ",
    //   is_active: "tab_two",
    //   type: 2,
    // },
  ];
  ...
  const getListElectricityRate = async (paramsSearch: any) => {
    ...
    const response = await SpaceTypeService.list(paramsSearch, abortController.signal);
  ...
  const titles = ["STT", "Loại căn hộ", "Vị trí hiển thị"];

  const dataFormat = ["text-center", "text-left", "text-center"];
  ...
  const onDelete = async (id: number) => {
    const response = await SpaceTypeService.delete(id);

    if (response.code === 0) {
      showToast("Xóa loại căn hộ thành công", "success");
      getListElectricityRate(params);
    } else {
      showToast(response.message ?? "Có lỗi xảy ra. Vui lòng thử lại sau", "error");
    }
  ...
  const showDialogConfirmDelete = (item?: any) => {
    const contentDialog: IContentDialog = {
      ...
      message: (
        <Fragment>
          Bạn có chắc chắn muốn xóa {item ? "loại căn hộ " : `${listIdChecked.length} loại căn hộ đã chọn`}
          {item ? <strong>{item.name}</strong> : ""}? Thao tác này không thể khôi phục.
        </Fragment>
      ),
      cancelText: "Hủy",
    ...
    <div className={`page-content page-branch${isNoItem ? " bg-white" : ""}`}>
      <div className="action-navigation">
        <div className="action-backup">
          ...
          />
          <h1 className="title-last">Danh sách loại căn hộ</h1>
        </div>
          ...
          <Fragment>
            {isPermissions ? (
              ...
              <SystemNotification
                description={
                  <span>
                    Hiện tại chưa có loại căn hộ nào. <br />
                    Hãy thêm mới loại căn hộ đầu tiên nhé!
                  </span>
                }
                type="no-item"
                titleButton="Thêm mới loại căn hộ"
                action={() => {
                  setDataElectricityRate(null);
                  setShowModalAdd(true);
                }}
