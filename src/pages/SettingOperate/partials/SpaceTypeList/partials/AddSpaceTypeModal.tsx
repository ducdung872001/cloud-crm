...
import "./AddSpaceTypeModal.scss";
import SpaceTypeService from "services/SpaceTypeService";

export default function AddSpaceTypeModal(props: any) {
  ...
  const onSubmit = async (e) => {
    ...
    const response = await SpaceTypeService.update(body);

    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} khu vực/loại căn hộ hộ thành công`, "success");
      onHide(true);
    } else {
    ...
    <Fragment>
      ...
      >
        <form className="form-electricity_rate-group" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader title={(data ? "Chỉnh sửa" : "Thêm mới") + " loại căn hộ"} toggle={() => !isSubmit && onHide(false)} />
          <ModalBody>
