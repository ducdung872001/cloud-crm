...
import AddSpaceModal from "./partials/AddSpaceModal";
import SpaceService from "services/SpaceService";

export default function SpaceList(props: any) {
  document.title = "Danh sách căn";

  const { onBackProps, dataSpace, setDataSpace, setShowModalAddSpace, reloadSpace, dataBuildingFloor, dataBuilding } = props;
  ...
  const listTabs = [
    {
      title:
        "Danh sách căn của tầng - " +
        (dataBuildingFloor ? ` ${dataBuildingFloor.name}` : "") +
        ", Toà - " +
        (dataBuilding ? `${dataBuilding.name}` : ""),
      is_active: "tab_one",
      type: 1,
    },
    // {
    //   title: "Sơ đồ phân cấp căn",
    //   is_active: "tab_two",
    //   type: 2,
    // },
  ];
  ...
  const getListSpace = async (paramsSearch: any) => {
    setIsLoading(true);
    const response = await SpaceService.list(paramsSearch, abortController.signal);
  ...
  const onDelete = async (id: number) => {
    ...
    if (response.code === 0) {
      showToast("Xóa căn thành công", "success");
      getListSpace(params);
    } else {
      showToast(response.message ?? "Có lỗi xảy ra. Vui lòng thử lại sau", "error");
    }
  ...
  const showDialogConfirmDelete = (item?: any) => {
    const contentDialog: IContentDialog = {
      ...
      message: (
        <Fragment>
          Bạn có chắc chắn muốn xóa {item ? "căn " : `${listIdChecked.length} căn đã chọn`}
          {item ? <strong>{item.name}</strong> : ""}? Thao tác này không thể khôi phục.
        </Fragment>
      ),
      cancelText: "Hủy",
          ...
          <Fragment>
            {isPermissions ? (
              ...
              <SystemNotification
                description={
                  <span>
                    Hiện tại chưa có căn nào. <br />
                    Hãy thêm mới căn đầu tiên nhé!
                  </span>
                }
                type="no-item"
                titleButton="Thêm mới căn"
                action={() => {
                  setDataSpace(null);
                  setShowModalAddSpace(true);
                }}
