...
export default function AddWaterRate(props: any) {
  ...
  const listFieldBasic = useMemo(
    () =>
      [
        {
          label: "Giới hạn dưới của mức tiêu thụ (m3)",
          placeholder: "Giới hạn dưới (m3)",
          name: "minUsage",
          type: "number",
          fill: true,
          required: true,
        ...
        {
          label: "Giới hạn trên của mức tiêu thụ (m3)",
          placeholder: "Giới hạn trên (m3)",
          name: "maxUsage",
          type: "number",
          fill: true,
          required: true,
        ...
        {
          label: "Giá mỗi m3 trong khoảng này (VNĐ)",
          placeholder: "Nhập giá mỗi m3 trong khoảng này (VNĐ/m3)",
          name: "ratePerM3",
          type: "number",
          fill: true,
          required: true,
  ...
  const onSubmit = async (e) => {
    ...
    const response = await WaterRateService.update(body);

    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} thang bậc giá nước thành công`, "success");
      onHide(true);
    } else {
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-add-water_rate"
      >
        <form className="form-water_rate-group" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader title={`${data ? "Chỉnh sửa" : "Thêm mới"} thang bậc giá nước`} toggle={() => !isSubmit && onHide(false)} />
          <ModalBody>
            <div className="list-form-group">
              <div className="form-basic-info-water_rate">
