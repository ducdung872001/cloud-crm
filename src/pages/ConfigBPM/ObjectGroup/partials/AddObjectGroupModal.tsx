...
export default function AddObjectGroupModal(props: any) {
  ...
  const onSubmit = async (e) => {
    ...
    const response = await ObjectGroupService.update(body);

    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} loại đối tượng thành công`, "success");
      onHide(true);
    } else {
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-add-object-group"
      >
        <form className="form-add-object-group" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader
            title={`${data ? "Chỉnh sửa" : "Thêm mới"} loại đối tượng`}
            toggle={() => {
