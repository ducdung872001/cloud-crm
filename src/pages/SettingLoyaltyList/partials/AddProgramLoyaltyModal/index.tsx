...
export default function AddProgramLoyaltyModal(props: AddProgramRoyaltyModalProps) {
  ...
  const values = useMemo(
    ...
    ({
      ...
      earnType: data?.earnType ?? "invoice_amount",
      earnRate: data?.earnRate ?? 10000,
      minSpendThreshold: data?.minSpendThreshold ?? 0,
      multiplierConfig: data?.multiplierConfig ?? "none",
  ...
  const listField = useMemo(
    () =>
      [
        ...
        {
          label: "Loại tích điểm",
          name: "earnType",
          type: "select",
          fill: true,
          options: [
            { label: "Theo tổng hoá đơn", value: "invoice_amount" },
            { label: "Theo danh mục sản phẩm", value: "category_based" },
            { label: "Cố định mỗi đơn", value: "fixed_per_order" },
          ],
        },
        {
          label: "Tỷ lệ quy đổi (VND → 1 điểm)",
          name: "earnRate",
          type: "number",
          fill: true,
        ...
        {
          label: "Chi tiêu tối thiểu để tích (VND)",
          name: "minSpendThreshold",
          type: "number",
          fill: true,
          placeholder: "VD: 50000 (đơn dưới 50k không tích)",
        },
        {
          label: "Nhân hệ số ngày đặc biệt",
          name: "multiplierConfig",
          type: "select",
          fill: true,
          options: [
            { label: "Không nhân hệ số", value: "none" },
            { label: "x2 cuối tuần", value: "weekend_2x" },
            { label: "x3 sinh nhật", value: "birthday_3x" },
            { label: "x2 cuối tuần + x3 sinh nhật", value: "weekend_birthday" },
          ],
        },
        {
          label: "Mô tả",
