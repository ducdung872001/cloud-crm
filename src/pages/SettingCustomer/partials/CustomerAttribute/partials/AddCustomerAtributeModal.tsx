...
import { AddCustomerAttributeModalProps } from "model/customerAttribute/PropsModel";
...
import "./AddCustomerAttributeModal.scss";
...
export default function AddCustomerAttributeModal(props: AddCustomerAttributeModalProps) {
  ...
  const values = useMemo(
    () =>
    ({
      custType: data?.custType?.toString() ?? "0",
      name: data?.name ?? "",
  ...
  const checkDuplicated = async (fieldName, custType, valueFieldName) => {
    const param = {
      id: 0,
      custType: custType,
      fieldName: fieldName
    }
    const response = await CustomerAttributeService.checkDuplicated(param);
    if(response.code === 0){
  ...
  useEffect(() => {
    if(onShow){
      checkDuplicated(formData.values?.fieldName, formData.values?.custType, values.fieldName)
    } 
  }, [formData.values?.fieldName, formData.values?.custType, values.fieldName, onShow ])
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-add-customer-attribute"
        size="lg"
      >
