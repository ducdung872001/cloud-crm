...
import "./CustomerMarketingLead.scss";
import CustomerMarketingLeadService from "services/CustomerMarketingLeadService";
import AddCustomerMarketingLeadModal from "./partials/AddCustomerMarketingLeadModal";

export default function CustomerMarketingLead(props: Record<string, unknown>) {
  document.title = "Nguồn Marketing gửi Lead";
  ...
  const getListCustomerSource = async (paramsSearch: ICustomerSourceFilterRequest) => {
    ...
    const response = await CustomerMarketingLeadService.list(paramsSearch, abortController.signal);
  ...
  const titles = ["STT", "Tên nguồn",  "Thứ tự hiển thị"];
  ...
  const onDelete = async (id: number) => {
    const response = await CustomerMarketingLeadService.delete(id);

    if (response.code === 0) {
      showToast("Xóa nguồn Marketing thành công", "success");
      getListCustomerSource(params);
    } else {
      showToast(response.message ?? "Có lỗi xảy ra. Vui lòng thử lại sau", "error");
    }
  ...
  const onDeleteAll = () => {
    ...
    const arrPromises = selectedIds.map((selectedId) => {
      ...
      if (found?.id) {
        return CustomerMarketingLeadService.delete(found.id);
      } else {
  ...
  const showDialogConfirmDelete = (item?: ICustomerSourceResponse) => {
    const contentDialog: IContentDialog = {
      ...
      message: (
        <Fragment>
          Bạn có chắc chắn muốn xóa {item ? "nguồn Marketing " : `${listIdChecked.length} nguồn marketing đã chọn`}
    ...
    <div className={`page-content page-customer-source${isNoItem ? " bg-white" : ""}`}>
      <div className="action-navigation">
        <div className="action-backup">
          ...
          />
          <h1 className="title-last">Nguồn Marketing gửi Lead</h1>
        </div>
          ...
          <Fragment>
            {isPermissions ? (
              ...
              <SystemNotification
                description={
                  <span>
                    Hiện tại chưa có nguồn Marketing nào. <br />
                    Hãy thêm mới nguồn Marketing đầu tiên nhé!
                  </span>
                }
                type="no-item"
