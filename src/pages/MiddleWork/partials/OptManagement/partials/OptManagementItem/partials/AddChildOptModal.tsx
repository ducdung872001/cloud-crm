...
import { IAddChildOptModal } from "model/workOpt/PropsModel";
import { IEmployeeFilterRequest } from "model/employee/EmployeeRequestModel";
import { IWorkOptRequestModel } from "model/workOpt/WorkOptRequestModel";
...
import "./AddChildOptModal.scss";
import { uploadDocumentFormData } from "utils/document";

export default function AddChildOptModal(props: IAddChildOptModal) {
  const { onShow, callBack, idOptManagement, idOpt } = props;
  ...
  const [detailParentOpt, setDetailParentOpt] = useState(null);

  // ở case này có đoạn call api WorkOptService 2 lần không phải là lỗi đâu : 1 lần là chi tiết con 1 lần chi tiết cha

  //! đoạn này lấy ra danh sách ảnh tài liệu dự án
  const [listImageWork, setListImageWork] = useState([]);

  const [dataOptChildren, setDataOptChildren] = useState<IWorkOptResponseModel>(null);

  const getDetailOptChildren = async (id: number) => {
  ...
  useEffect(() => {
    if (onShow && idOpt && idOptManagement) {
      getDetailOptChildren(idOpt);
    }
  }, [onShow, idOpt, idOptManagement]);

  const values = useMemo(
    () =>
      ({
        name: dataOptChildren?.name ?? "",
        code: dataOptChildren?.code ?? "",
        startTime: dataOptChildren?.startTime ?? new Date(),
        endTime: dataOptChildren?.endTime ?? "",
        description: dataOptChildren?.description ?? "",
        // participants: dataOptChildren?.participants ?? "[]",
        employeeId: dataOptChildren?.employeeId ?? null,
        docLink: JSON.parse(dataOptChildren?.docLink || "[]") ?? [],
        parentId: idOptManagement,
      } as IWorkOptRequestModel),
    [dataOptChildren, onShow, idOptManagement]
  );
  ...
  const getDetailParentOpt = async () => {
  ...
  useEffect(() => {
    if (detailParentOpt) {
      setFormData({ ...formData, values: { ...formData?.values, parentId: detailParentOpt?.value } });
    }
  }, [detailParentOpt]);
  ...
  const handleChangeValueParticipant = (dataOptChildren) => {
    setDataParticipant(dataOptChildren);

    const result = (dataOptChildren || []).map((item) => +item.value);
  ...
  const getDetailEmployee = async () => {
    const response = await EmployeeService.detail(dataOptChildren?.employeeId);
  ...
  useEffect(() => {
    if (dataOptChildren?.id && onShow) {
      getDetailEmployee();
    }
  }, [dataOptChildren?.id, onShow]);
  ...
  useEffect(() => {
    if (onShow && dataOptChildren === null) {
      getDetailEmployeeInfo();
    }
  }, [dataOptChildren, onShow]);

  //! đoạn này xử lý vấn đề nếu như mà có tài liệu lúc update thì view ra giao diện
  useEffect(() => {
    if (dataOptChildren && dataOptChildren.docLink) {
      const result = JSON.parse(dataOptChildren.docLink || "[]");
  ...
  const onSubmit = async (e) => {
    ...
    const body: IWorkOptRequestModel = {
      ...(formData.values as IWorkOptRequestModel),
      ...(dataOptChildren ? { id: dataOptChildren.id } : {}),
    ...
    if (response.code === 0) {
      showToast(`${dataOptChildren ? "Cập nhật" : "Thêm mới"} thư mục dự án thành công`, "success");
      callBack(true);
  ...
  const actions = useMemo<IActionModal>(
    () => ({
      actions_right: {
        buttons: [
          ...
          {
            title: idOptManagement ? "Cập nhật" : "Tạo mới",
            type: "submit",
            color: "primary",
            disabled:
              isSubmit ||
    ...
    [formData, values, isSubmit, startDay, endDay, idOptManagement]
  );
  ...
  const showDialogConfirmCancel = () => {
    const contentDialog: IContentDialog = {
      ...
      isCentered: true,
      isLoading: false,
      title: <Fragment>{`Hủy bỏ thao tác ${dataOptChildren ? "chỉnh sửa" : "thêm mới"}`}</Fragment>,
    ...
    <Fragment>
      ...
      >
        <form className="form-add-child-project" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader
            title={`${dataOptChildren ? "Chỉnh sửa" : "Thêm mới"} dự án`}
            toggle={() => {
