...
import { AddOfferProductModalProps } from "model/offerProduct/PropsModel";
import { IActionModal, IOption } from "model/OtherModel";
import { IListWarehouseProductFilterRequest } from "model/warehouse/WarehouseRequestModel";
import { IEmployeeFilterRequest } from "model/employee/EmployeeRequestModel";
import { IFieldCustomize, IFormData, IValidation } from "model/FormModel";
import { IOfferProductRequest } from "model/offerProduct/OfferProductRequestModel";
...
import "./AddOfferProductModal.scss";
...
export default function AddofferProductModal(props: any) {
      ...
      ({
        ...
        priceSample:
          data?.discountUnit == 0
            ...
            : 0,
      } as IOfferProductRequest),
    [data, onShow, idCustomer, offerId]
  );
        ...
        },
        {
          ...
          snippet: (
            <SelectCustom
              ...
              isAsyncPaginate={true}
              isFormatOptionLabel={true}
              placeholder="Chọn người báo giá"
              additional={{
                page: 1,
              }}
              ...
              message="Vui lòng chọn người báo giá"
            />
          ),
        },
        {
  ...
  const onSubmit = async (e) => {
    ...
    const body: IOfferProductRequest = {
      ...(formData?.values as IOfferProductRequest),
      ...(data ? { id: data.id } : {}),
    };
    ...
    const response = await offerProductService.update(body);

    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} sản phẩm báo giá thành công`, "success");
      onHide(true);
      setDataProduct(null);
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-add-offer-product"
      >
        <form className="form-add-offer-product" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader
            title={`${data ? "Chỉnh sửa" : "Thêm mới"} sản phẩm báo giá`}
            toggle={() => {
