...
export default function ModalAddColumn({ onShow, onHide, indexColumn, processId, setListColumn, listKeyColumn, listColumn }) {
  ...
  const valueKeyOld = useRef(null);

  useEffect(() => {
    if (onShow && indexColumn) {
      const column = listColumn[indexColumn];
      if (column) {
        valueKeyOld.current = column.key;
      }
    }
  }, [onShow]);
    ...
    () => ({
      actions_right: {
        buttons: [
          ...
          {
            title: indexColumn ? "Áp dụng" : "Thêm cột",
            type: "submit",
            color: "primary",
            disabled: isSubmit,
            is_loading: isSubmit,
        ...
        <div className="form-mapping">
          <div className="container-header">
            <div className="box-title">
              <h4>{indexColumn ? "Sửa " : "Thêm "}cột điều kiệnn</h4>
            </div>
          </div>
          <ModalBody>
                            ...
                            <Input
                              ...
                              error={
                                indexColumn
                                  ? formData?.key &&
                                    valueKeyOld?.current &&
                                    valueKeyOld?.current != formData?.key &&
                                    listKeyColumn.includes(formData.key)
                                  : formData?.key && listKeyColumn.includes(formData.key)
                              }
