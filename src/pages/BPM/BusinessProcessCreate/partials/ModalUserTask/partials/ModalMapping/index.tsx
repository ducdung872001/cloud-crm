...
export default function ModalMapping({ onShow, onHide, dataNode, listComponent, processId, codeForm }) {
  ...
  const extractKeysAndPaths = (components) => {
    ...
    components.forEach((component) => {
      ...
      if (component.path) {
        if (component.type === "group" || component.type === "dynamiclist") {
          result.push({
  ...
  const onSaveMapping = async (e) => {
    ...
    const body = {
      ...
      fieldNameType: formData.fieldNameType,
                      ...
                      <div key={index} className="container-mapping">
                        <div className="form-group">
                          {tabMapping === "input" ? (
                            ...
                            >
                              {!formData.fieldNameType && idxEditMapping === index ? (
                                <div className="input-text">
                                  <Input
                                    ...
                                    disabled={item.id ? (item.id === idEditMapping ? false : true) : idxEditMapping === index ? false : true}
                                    onChange={(e) => {
                                      setFormData({ ...formData, fieldName: e.target.value, fieldLabel: e.target.value });
                                    }}
                              ...
                              {(idxEditMapping === index || item.id === idEditMapping) && !formData.fieldNameType ? null : (
                              ...
                              {/* {tabMapping === 'input' ?  */}
                              <Tippy content={formData.fieldNameType === 0 ? "Chuyển chọn trường trong form" : "Chuyển nhập giá trị"}>
                                <div
                                  className={
                                    item.id
                                      ? item.id === idEditMapping
                                  ...
                                  onClick={(e) => {
                                    if (item.id) {
                                      if (item.id === idEditMapping) {
                                        setFormData({ ...formData, fieldNameType: formData.fieldNameType === 0 ? 1 : 0 });
                                      }
                                    } else {
                        ...
                        <div className="form-group">
                          ...
                          >
                            {!formData.mappingType && idxEditMapping === index ? (
                              <div className="input-text">
                                <Input
                                  name="fielName"
                                  fill={false}
                                  value={formData.fromFieldName}
                                  // disabled={disableFieldCommom}
                                  onChange={(e) => {
                                    setFormData({ ...formData, fromNodeId: "", fromFieldName: e.target.value });
                                  }}
                                  placeholder={`Nhập giá trị`}
                                />
                              </div>
                            ...
                            {(idxEditMapping === index || item.id === idEditMapping) && !formData.mappingType ? null : (
                              <div className="select-mapping">
                                <SelectCustom
