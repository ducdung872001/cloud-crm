...
export default function ModalParallelGatewayTask({ onShow, onHide, dataNode, processId, disable }) {
  ...
  const getDetailTask = async (id) => {
    const response = await BusinessProcessService.detailParallelGateway(id);

    if (response.code == 0) {
      const result = response.result;

      const triggerConditions = (result?.triggerConditions && JSON.parse(result.triggerConditions)) || null;
      setDataTriggerConditions(triggerConditions?.config || { quantity: "", quantityType: 0 });

      /////
      const newListTriggerConditionData =
        result?.triggerConditions &&
        JSON.parse(result.triggerConditions) &&
        Array.isArray(JSON.parse(result.triggerConditions)) &&
        JSON.parse(result.triggerConditions).length > 0
          ? JSON.parse(result.triggerConditions)
          : [];
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-parallel-gateway"
      >
        <form className="form-parallel-gateway" onSubmit={(e) => onSubmit(e)}>
          {/* <ModalHeader title={`Cài đặt cổng`} toggle={() => !isSubmit && handleClear(false)} /> */}
          <div className="container-header">
            <div className="box-title">
              <h4>{"Cài đặt Parallel Gateway"}</h4>
            </div>
              ...
              <div className="container-trigger-condition">
                <RadioList
                  ...
                  onChange={(e) => {
                    ...
                    if (value === "FLOW_NUM") {
                      setDataTriggerConditions({ quantity: "", quantityType: 0 });
                    }
                ...
                {formData.triggerConditions === "FLOW_NUM" ? (
                  <div>
                    <span style={{ fontSize: 14, fontWeight: "700", marginBottom: 10 }}>Số luồng tới</span>
                  </div>
                ) : null}

                {formData.triggerConditions === "FLOW_NUM" ? (
                  <div className="continer-flow-num">
                    {!dataTriggerConditions?.quantityType ? (
                      <div className="input-text">
                        <Input
                          ...
                          onChange={(e) => {
                            const value = e.target.value;
                            setDataTriggerConditions({ quantity: value, quantityType: 0 });
                          }}
                      ...
                      <div className="select-mapping">
                        <SelectCustom
                          key={dataTriggerConditions?.quantityType}
                          id="fieldName"
                          name="fieldName"
                          ...
                          options={[]}
                          value={
                            dataTriggerConditions?.quantity
                              ? { value: dataTriggerConditions?.quantity, label: dataTriggerConditions?.quantity }
                              : null
                          }
                          onChange={(e) => {
                            setDataTriggerConditions({ ...dataTriggerConditions, quantity: e.value });
                          }}
                          isAsyncPaginate={true}
                          isFormatOptionLabel={false}
                          // placeholder={item.mappingType === 2 ? "Chọn biến" : 'Chọn trường trong form'}
                          placeholder={dataTriggerConditions?.quantityType === 2 ? "Chọn biến" : "Chọn trường trong form"}
                          additional={{
                            page: 1,
                          }}
                          loadOptionsPaginate={dataTriggerConditions?.quantityType === 2 ? loadedOptionAttribute : loadedOptionForm}
                    ...
                    <Tippy
                      content={
                        dataTriggerConditions?.quantityType === 0
                          ? "Chuyển chọn trường trong form"
                          : dataTriggerConditions?.quantityType === 1
                          ? "Chuyển chọn biến"
                          : "Chuyển nhập giá trị"
                      }
                    >
                      <div
                        ...
                        onClick={(e) => {
                          setDataTriggerConditions({
                            quantity: "",
                            quantityType: dataTriggerConditions?.quantityType === 0 ? 1 : dataTriggerConditions?.quantityType === 1 ? 2 : 0,
                          });
                        }}
                      >
