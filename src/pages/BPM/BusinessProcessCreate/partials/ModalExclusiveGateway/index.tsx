...
export default function ModalExclusiveGateway({ onShow, onHide, dataNode, processId, disable }) {
  ...
  // console.log('dataTriggerConditions', dataTriggerConditions);
  ...
  const getDetailTask = async (id) => {
    const response = await BusinessProcessService.detailExclusiveGateway(id);
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-exclusive-gateway"
      >
        <form className="form-exclusive-gateway" onSubmit={(e) => onSubmit(e)}>
          {/* <ModalHeader title={`Cài đặt Gateway`} toggle={() => !isSubmit && handleClear(false)} /> */}
          <div className="container-header">
            <div className="box-title">
              <h4>{"Cài đặt Exclusive Gateway"}</h4>
            </div>
            <ListButtonHeader
              data={data}
              dataNode={dataNode}
