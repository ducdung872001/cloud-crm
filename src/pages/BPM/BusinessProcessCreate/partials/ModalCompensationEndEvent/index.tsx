...
export default function ModalCompensationEndEvent({ onShow, onHide, dataNode, processId, changeNameNodeXML, disable }) {
  ...
  const getDetailTask = async (id) => {
    const response = await BusinessProcessService.detailCompensationEndEvent(id);
    if (response.code == 0) {
      const result = response.result;
      setData(result);
  ...
  const onSubmit = async (e) => {
    ...
    const response = await BusinessProcessService.updateCompensationEndEvent(body);
    if (response.code === 0) {
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-compensation-end-event"
      >
        <form className="form-compensation-end" onSubmit={(e) => onSubmit(e)}>
          <div className="container-header">
            <div className="box-title">
              <h4>{"Cài đặt Compensation End Event"}</h4>
            </div>
            <ListButtonHeader
              data={data}
              dataNode={dataNode}
