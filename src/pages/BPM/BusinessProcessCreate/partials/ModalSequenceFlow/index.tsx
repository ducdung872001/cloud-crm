...
export default function ModalSequenceFlow(props: any) {
      ...
      dataOption &&
        ...
        dataOption.map((item) => {
          ...
          components.map((el) => {
            ...
            // }
            if (el.key) {
              if (el.type === "checklist" && el.values && el.values.length > 0) {
                el.values.map((il) => {
                  listForm.push({
                    value: `frm_${item.code}.${el.key}.${il.value}`,
                    label: `frm_${item.code}.${el.key}.${il.value}`,
                    nodeId: item.nodeId,
              ...
              if (el.components && el.components.length > 0) {
                el.components.map((il) => {
                  listForm.push({
                    value: `frm_${item.code}.${el.path}.${il.key}`,
                    label: `frm_${item.code}.${el.path}.${il.key}`,
                    nodeId: item.nodeId,
                    datatype: il.type || null,
                  });
                });
  ...
  const onSubmit = async (formData) => {
    ...
    const body = {
      ...
      flowType: formData.rule?.filter((el) => !el.fieldName).length === formData.rule?.length ? "normal" : "condition",
      priority: priority,
    };
  ...
  const actions = useMemo<IActionModal>(
    ...
    [isSubmit, nodeName, dataNode, formData, disable, priority]
  );
  ...
  const addLinkNode = async () => {
    ...
    const body = {
      ...
      fromNodeId: fromNodeId,
      toNodeId: toNodeId,
      flowType: "condition", //normal, condition
      config: JSON.stringify(configDataNew),
      processId: processId,
      linkId: dataNode.id,
    };
            ...
            <div className="container-sequence-flow">
              <div className="box-priority">
                <div style={{ marginRight: 10 }}>
                  <span style={{ fontSize: 14, fontWeight: "700" }}>Mức độ ưu tiên: </span>
                </div>
                <NummericInput
                  name="priority"
                  fill={false}
