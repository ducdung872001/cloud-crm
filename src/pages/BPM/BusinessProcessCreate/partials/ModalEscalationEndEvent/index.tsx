...
export default function ModalEscalationEndEvent({ onShow, onHide, dataNode, processId, changeNameNodeXML, disable }) {
  ...
  const getDetailTask = async (id) => {
    const response = await BusinessProcessService.detailEscalationEndEvent(id);
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-escalation-end-event"
      >
        <form className="form-escalation-end-event" onSubmit={(e) => onSubmit(e)}>
          <div className="container-header">
            <div className="box-title">
              <h4>{"Cài đặt Escalation end event"}</h4>
            </div>
            <ListButtonHeader
              data={data}
              dataNode={dataNode}
