...
import { ISendZaloChatRequest } from "model/zaloOA/ZaloOARequest";
...
export default function MessageChat(props: IMessageChatProps) {
  ...
  const [messageChat, setMessageChat] = useState<ISendZaloChatRequest>({
    userIdByApp: data.userIdByApp,
    message: "",
  });

  useEffect(() => {
    setMessageChat({ ...messageChat, userIdByApp: data?.userIdByApp, message: "" });
  }, [data]);
  ...
  const handleSendChat = async (e) => {
    ...
    const body: ISendZaloChatRequest = {
      ...(messageChat as ISendZaloChatRequest),
    };

    const response = await ZaloOAService.sendZaloChat(body);
  ...
  const handleChangeValueContent = (e) => {
    ...
    if (!value.startsWith(" ")) {
      setMessageChat({ ...messageChat, message: value });
    }
  };
  ...
  useLayoutEffect(() => {
    ...
    // Set height
    refTextarea.current.style.height = messageChat.message.length >= 73 ? `${Math.max(refTextarea.current.scrollHeight, 34)}px` : "34px";
  ...
  }, [totalHeight, messageChat.message]);
  ...
  return (
    <Fragment>
      <div className="message-chat">
        ...
        <div className={`${messageChat.message ? "active__send--message" : "hide__send--message"}`} onClick={(e) => handleSendChat(e)}>
