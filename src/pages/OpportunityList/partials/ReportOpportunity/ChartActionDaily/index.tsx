...
export default function ChartActionDaily(props: ReportRevenueProps) {
  const { classNames, paramsProps, dataSuccessOpportunity, listDate, timestampArray } = props;
  ...
  useEffect(() => {
    setChartData({
      ...
      series: [
        {
          name: "Số lượng cơ hội thành công",
          data: dataSuccessOpportunity.map((item, index) => [timestampArray[index], item]),
          color: chartColor.success,
        },
      ],
    });
  }, [dataSuccessOpportunity, listDate]);
  ...
  const [chartData, setChartData] = useState({
    chart: {
      type: "column",
      zoomType: "x", // Cho phép zoom theo trục x.
    },
    title: {
      text: "",
    },
    ...
    series: [
      ...
      // {
      //   name: "Số lượng cơ thành công",
      //   data: [40, 200, 50, 250, 100],
      //   color: chartColor.success,
      // },
    ],
    exporting: {
  ...
  return (
    <div className={`card-box report-revenue${classNames ? ` ${classNames}` : ""}`}>
      <div className="title__common d-flex align-items-start">
        <h2 className="name-common">Số lượng cơ hội thành công</h2>
        <div
