...
export default function ChartOptNewDaily(props: ReportRevenueProps) {
  const { classNames, paramsProps, dataNewOpportunity, listDate, timestampArray } = props;
  ...
  useEffect(() => {
    setChartData({
      ...
      series: [
        {
          name: "Số lượng cơ hội thêm mới",
          data: dataNewOpportunity.map((item, index) => [timestampArray[index], item]),
          color: chartColor.success,
        },
      ],
    });
  }, [dataNewOpportunity, listDate]);
  ...
  return (
    <div className={`card-box report-revenue${classNames ? ` ${classNames}` : ""}`}>
      <div className="title__common d-flex align-items-start">
        <h2 className="name-common">Số lượng cơ hội thêm mới</h2>
        <div
