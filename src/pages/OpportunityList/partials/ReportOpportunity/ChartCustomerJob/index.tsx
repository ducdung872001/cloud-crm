...
export default function ChartCustomerJob(props) {
  const { paramsProps, dataTotalByApproach } = props;
  ...
  useEffect(() => {
    if (dataTotalByApproach) {
      const listKey = Object.keys(dataTotalByApproach);
      setChartData({
        ...chartData,
        series: [
          {
            name: "",
            colorByPoint: true,
            data: listKey.map((key) => ({
              name: key,
              y: dataTotalByApproach[key],
              text: `${dataTotalByApproach[key]} cơ hội`,
            })),
          },
        ],
      });
  ...
  return (
    <div className="card-box page__customer--campaign chart-report__detail">
      <div className="title__common d-flex align-items-start">
        <h2 className="name-common">Số cơ hội đang theo</h2>
        <div
