...
export default function TableDocDefinition(props: TableDocDefinitionProps) {
  const { info, title, header, items, mapFooter, widths, customFooter, options, customFieldExportPDF } = props;
  ...
  const itemsFinal = items.map((item) => {
    ...
    return item.map((e) => {
      ...
      if (typeof e === "number") {
        return { text: formatCurrency(+e, ","), alignment: "right" };
      }

      if (typeof e?.text === "number") {
        return {
          ...e,
          alignment: "right",
          text: formatCurrency(+e.text, ","),
        };
      }
    ...
    content: [
      {
        columns: [
          ...
          },
          {
            ...
            ul: [
              {
                text: info.product_store?.name,
                style: "header",
                alignment: "center",
              },
              {
                text: ["Địa chỉ: ", { text: info.product_store?.address, bold: true }],
                alignment: "center",
              },
              {
                text: ["Điện thoại: ", { text: info.product_store?.phone, bold: true }],
                alignment: "center",
              },
            ],
          },
      ...
      "\n",
      {
        layout: options.smallTable ? "customTableLayoutSmall" : "customTableLayout",
        lineHeight: 1.3,
        table: {
          headerRows: 1,
          widths: widthsFinal,
