...
export async function ExportExcel(userOptions, product_store?: Record<string, unknown>, name?: string) {
  const options = {
    title: "",
    titleExtra: [],
    titleExtraFormat: [],
  ...
  workbook.creator = process.env.APP_TYPE === "GDP" ? "SPHACY GDP" : "SPHACY GPP";
  ...
  // add row to create new product (formImport)
  if (options.isFormImport) {
    [...Array(2000)].forEach(() => {
      const row = worksheetAddRow(
        worksheet,
