...
export default function TagsInput(props: TagsInputProps) {
  ...
  const addTagData = (e) => {
    e.preventDefault();
    if (e.target.value.trim() !== "" && e.target.value.trim().match(validateRegex)) {
      if (!tagsData?.some((tag) => tag === e.target.value.trim())) {
        addTag([...tagsData, e.target.value.trim()]);
      }
      e.target.value = "";
      setErrorShow(false);
    } else if (e.target.value.trim() !== "" && !e.target.value.trim().match(validateRegex)) {
      setErrorShow(true);
    }
  };
    ...
    if (e.clipboardData.getData("Text").trim() !== "") {
      ...
      if (listTag.length > 0) {
        ...
        for (let i = 0; i < listTag.length; i++) {
          if (!tagsData.some((tag) => tag === listTag[i]) && listTag[i].match(validateRegex)) {
  ...
  return (
    <div className="react-tag-input" onClick={() => focusToInput()}>
      <div
        className={`react-tag-input__container${focusInput ? " on-focused" : ""}${tagsData?.length > 0 ? " has-value" : ""}${
          valueInput.length > 0 ? " has-value-input" : ""
        }`}
      >
