...
const NAV_SALES: NavItem[] = [
  ...
  {
    page: "leads", label: "Lead Management", badge: 24,
  ...
  {
    page: "pipeline", label: "Pipeline & Cơ hội", badge: 11,
    icon: <svg viewBox="0 0 24 24"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>,
  },
  {
    page: "campaigns", label: "Chiến dịch", badge: 3, badgeColor: "gold",
    icon: <svg viewBox="0 0 24 24"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg>,
  },
  {
    page: "salesprocess", label: "Quy trình bán", badge: "Admin", badgeColor: "purple",
    icon: <svg viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 21V9"/></svg>,
  },
  {
    page: "salesdocs", label: "Tài liệu bán hàng",
...
const NAV_OPS: NavItem[] = [
  {
    page: "approval", label: "Phê duyệt hồ sơ", badge: 7,
    icon: <svg viewBox="0 0 24 24"><polyline points="9 11 12 14 22 4"/><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/></svg>,
  },
  {
    page: "kpi", label: "Báo cáo KPI",
...
const NAV_SYSTEM: NavItem[] = [
  {
    modal: "modal-corebankingsync", label: "Core Banking Sync",
    icon: <svg viewBox="0 0 24 24"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 11-2.12-9.36L23 10"/></svg>,
  },
  {
    modal: "modal-settings", label: "Cài đặt",
...
function NavItemRow({ item }: { item: NavItem }) {
  ...
  return (
    ...
    >
      ...
      {!isCollapsedSidebar && item.badge != null && (
...
export default function Sidebar() {
  const { user, isCollapsedSidebar, openModal, setActivePage } = useApp();

  return (
    <>
      {/* Mobile overlay */}
      <div
        className={`sidebar-overlay${isCollapsedSidebar ? " sidebar-overlay--show" : ""}`}
        onClick={() => {}}
      />
      ...
      <aside className={`sidebar${isCollapsedSidebar ? " sidebar--collapsed" : ""}`}>
        ...
        <div className="nav-section">
          ...
          {NAV_SALES.map((item) => (
            <NavItemRow key={item.page || item.modal} item={item} />
          ))}
        </div>
        ...
        <div className="nav-section">
          ...
          {NAV_SYSTEM.map((item) => (
            <NavItemRow key={item.modal} item={item} />
          ))}
        </div>
