...
Owner-entity rule (per memory `project_taxonomy_owner_entity_rule.md` + handoff đã chốt `cloud-sales-master#23`): course là **service** thuộc sales, nên **session = instance lịch của 1 service** cũng do `sales` sở hữu. Không reroute sang inventory/integration.
